"""
LSTM v8 - Feature-Attention LSTM (FA-LSTM) with very small window.

Motivated by Wang et al. 2024 (HESS): "Feature attention and adversarial
training significantly enhance model stability" for 7-day forecasts. We
implement the FA part: a per-timestep softmax-weighted gate over the input
feature vector, multiplied into the features before the LSTM. The gate is
shared across timesteps but produces dynamic per-timestep weights.

Window is intentionally short (7 days) to match the regime where lit shows
LSTMs do best (East Java 2025: smaller windows; Wang 2024: 7-day target).
The same pruned feature set as v7 is used so the comparison isolates the
attention mechanism + window length.

Usage:
    python -m Models.Temporal.lstm.train_v8
"""
...
OUT_DIR  = Path(__file__).parent / "outputs_v8"
...
class FeatureAttention(nn.Module):
    """Compute a softmax weight per feature per timestep."""
    ...
    def forward(self, x):
        # x: (B, S, F); softmax over feature axis
        logits = self.gate(x)
        weights = torch.softmax(logits, dim=-1)
        # scale up by F so the average mass is 1 (preserves magnitudes)
        return x * weights * x.size(-1), weights


class FALSTM(nn.Module):
    """Feature-attention LSTM: per-timestep feature gate -> LSTM -> head."""

    def __init__(self, n_features, hidden_size, num_layers, dropout, attn_hidden):
        super().__init__()
...
def save_loss_curve(train_losses, val_losses):
    try:
        ...
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
        ax.set_title(f"FA-LSTM v8 (seq_len={SEQ_LEN})")
        ax.legend()
        fig.tight_layout()
...
def main():
    ...
    print(f"[v8] FA-LSTM  {len(ALL_FEATURES)} features  seq_len={SEQ_LEN}")
    ...
    print(f"[model] FALSTM  params={n_params:,}")
    ...
    results["config"] = dict(
        variant="v8_fa_lstm_short_window",
        seq_len=SEQ_LEN, train_stride=TRAIN_STRIDE,
        hidden_size=HIDDEN_SIZE, num_layers=NUM_LAYERS, dropout=DROPOUT,
        attn_hidden=ATTN_HIDDEN, batch_size=BATCH_SIZE, lr=LR,
        weight_decay=WEIGHT_DECAY, huber_delta=HUBER_DELTA,
        scheduler="cosine_warm_restarts(T_0=20,T_mult=2)",
