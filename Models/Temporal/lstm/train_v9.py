"""
LSTM v9 - Bidirectional LSTM + temporal attention pooling, mid-window,
with a small set of high-value rolling/lag features added in-sequence.

Motivated by:
  - East Java 2025 (mid windows still useful for multi-day horizons).
  - Hebei Hybrid 2024 (Transformer-style attention over sequence helps
    long-range dependencies). We use BiLSTM + attention pooling as a
    lightweight alternative to a full transformer.
  - 4/29 Report Table 3: a small subset of XGBoost-favorite rolling/lag
    features tend to help when added without dominating. We add the
    top-3 (SMAP_sm_pm_interp_ema02, V_ema_LST_modis_kobs7,
    V_rollmean_G_API_kobs14). Limited to 3 to avoid the collapse pattern
    that ~496 lag features previously caused.

Window is 10 days -- inside the 5-15d sweet spot from the lit review.

Usage:
    python -m Models.Temporal.lstm.train_v9
"""
...
OUT_DIR  = Path(__file__).parent / "outputs_v9"
...
TIME_FEATURES = [
    ...
    "SMAP_sm_interp_mask",
    "sin_year", "cos_year",
    # Selected rolling/lag features (top-3 from XGBoost importance, no G_ raws)
...
def save_loss_curve(train_losses, val_losses):
    try:
        ...
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
        ax.set_title(f"BiLSTM+Attn v9 (seq_len={SEQ_LEN})")
        ax.legend()
        fig.tight_layout()
...
def main():
    ...
    print(f"[v9] BiLSTM+Attn  {len(TIME_FEATURES)} time + {len(STATIC_FEATURES)} static  seq_len={SEQ_LEN}")
    ...
    results["config"] = dict(
        variant="v9_bilstm_attn_with_top_lags",
