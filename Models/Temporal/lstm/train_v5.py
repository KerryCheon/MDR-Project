"""
LSTM training pipeline v5 — Hybrid: top XGBoost rolling/lag features
added to the raw time sequence.

Hypothesis
==========
The LSTM's bottleneck is that it must learn temporal patterns (lag effects,
wetting/drying dynamics) from raw daily observations, while XGBoost gets
those patterns handed to it via pre-computed rolling means, EMAs, and lags.

v5 gives the LSTM a head start by including the 10 most important
pre-computed rolling/lag features from the XGBoost importance ranking
(final_set_feb_19.json) alongside the original 21 raw observations.
This is a feature-engineering change — the architecture stays the same
LSTMRawSeries at ~217K params.

Changes from v2 (the 21-feature baseline with LSTMRawSeries):
  1. TIME_FEATURES extended from 21 -> 31 (10 new rolling/lag/ema features)
  2. ReduceLROnPlateau patience=25
  3. MAX_EPOCHS=300, PATIENCE=60
  4. Outputs saved to Models/Temporal/lstm/outputs_v5/

The 10 new features (ranked by XGBoost importance):
  SMAP_sm_pm_interp_ema02          (rank 1 — SMAP PM exponential moving avg)
  V_rollmin_LST_modis_kobs30       (rank 2 — LST 30-day rolling minimum)
  V_rollmean_G_API_kobs14          (rank 11 — API 14-day rolling mean)
  V_rollmean_s2_b11_kobs7          (rank 20 — SWIR band 7-day rolling mean)
  V_ema_LST_modis_kobs7            (rank 21 — LST 7-day EMA)
  SMAP_sm_am_interp_rollrange7     (rank 38 — SMAP AM 7-day rolling range)
  V_rollstd_F_NDMI_kobs7           (rank 43 — NDMI 7-day rolling std)
  V_rollstd_E_SAR_ratio_kobs7      (rank 44 — SAR ratio 7-day rolling std)
  V_rollstd_F_NDVI_kobs7           (rank 52 — NDVI 7-day rolling std)
  SMAP_sm_interp_rollstd7          (rank 76 — SMAP combined 7-day rolling std)

NOTE: Raw G_ features (G_API, G_DSLR, G_rain_sum_*) are NOT added — previous
experiments showed they cause catastrophic collapse even with log1p.

Usage:
    python -m Models.Temporal.lstm.train_v5

Outputs (written to Models/Temporal/lstm/outputs_v5/):
...
# v5: original 21 features + 10 top XGBoost rolling/lag features.
TIME_FEATURES = [
    # === Original 21 raw daily observations ===
    ...
    "sin_year", "cos_year",
    # === NEW in v5: 10 top XGBoost rolling/lag features ===
    # SMAP rolling features
    "SMAP_sm_pm_interp_ema02",          # rank 1  — PM soil moisture EMA (alpha=0.2)
    "SMAP_sm_am_interp_rollrange7",     # rank 38 — AM 7-day rolling range
    "SMAP_sm_interp_rollstd7",          # rank 76 — combined 7-day rolling std
    # LST rolling features
    "V_rollmin_LST_modis_kobs30",       # rank 2  — 30-day rolling minimum LST
    "V_ema_LST_modis_kobs7",            # rank 21 — 7-day EMA of LST
    # API rolling feature (safe — this is rolling mean, not raw G_API)
    "V_rollmean_G_API_kobs14",          # rank 11 — 14-day rolling mean API
    # Sentinel-2 SWIR rolling feature
    "V_rollmean_s2_b11_kobs7",          # rank 20 — 7-day rolling mean SWIR
...
# ---------------------------------------------------------------------------
# Hyperparameters — architecture identical to v2 baseline (~217K params)
# ---------------------------------------------------------------------------
SEQ_LEN          = 60
TRAIN_STRIDE     = 1
TIME_PROJ_SIZE   = 32       # LSTM input projection (n_time -> 32)
STATIC_PROJ_SIZE = 32
HIDDEN_SIZE      = 128
NUM_LAYERS       = 2
DROPOUT          = 0.3
...
def save_loss_curve(train_losses: list, val_losses: list):
    try:
        ...
        ax.set_title("LSTM v5 — Hybrid (21 raw + 10 XGBoost rolling/lag) Training Curve")
        ax.legend()
        fig.tight_layout()
...
def main():
    ...
    print(f"[features] {len(TIME_FEATURES)} time  +  {len(STATIC_FEATURES)} static  =  {len(all_cols)} total")
    print(f"[v5] 21 raw + 10 XGBoost rolling/lag features in time sequence")
    ...
    # Model — same LSTMRawSeries, just with 31 time features instead of 21
    model = LSTMRawSeries(
    ...
    print(f"[model] LSTMRawSeries  params={n_params:,}")
    ...
    results["config"] = dict(
        variant="v5_hybrid_xgboost_rolling_features",
