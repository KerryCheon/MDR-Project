"""
LSTM v18 - Rolling window split: train 2017-2021, val 2022, test 2023-2025.

Identical BiLSTMAttn architecture to v9. Only the split changes: val years
are absorbed into training and 2022 becomes the early-stop val year. Same
test holdout (2023-2025) allows direct R^2 comparison to v9 (0.747).

Motivation: audit_split.py found SMAP features ~20% OOD in test vs train.
The test years (2023-2025) have higher SMAP values than training years
(2017-2020). Including 2021 in training exposes the model to a wider SMAP
distribution.

Usage:
    python -m Models.Temporal.lstm.train_v18
"""
...
OUT_DIR  = Path(__file__).parent / "outputs_v18"
...
def main():
    ...
    # Load all splits and re-partition by year
    all_df = pd.concat([
    ...
    train_df = all_df[all_df["year"].isin(range(2017, 2022))].copy()  # 2017-2021
    val_df   = all_df[all_df["year"] == 2022].copy()
    test_df  = all_df[all_df["year"].isin(range(2023, 2026))].copy()  # 2023-2025

    print(f"[split] train={train_df.shape}  val={val_df.shape}  test={test_df.shape}")
    ...
    print(f"[v18] BiLSTM+Attn  {len(TIME_FEATURES)} time + {len(STATIC_FEATURES)} static  seq_len={SEQ_LEN}")
    ...
    try:
        ...
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
        ax.set_title(f"BiLSTM+Attn v18 rolling window (train 2017-2021 / val 2022 / test 2023-2025)")
        ax.legend()
        fig.tight_layout()
