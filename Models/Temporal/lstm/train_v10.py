"""
LSTM v10 - v9 + heavier regularization to close the train/test gap.

v9 hit train R^2=0.93 vs test R^2=0.75 — clear overfitting. v10 keeps the
BiLSTM + temporal attention pooling architecture and the same in-sequence
rolling features, but pushes harder on regularization:

  - Input Gaussian noise during training (std=0.02 on the standardized
    feature space) -- a cheap form of data augmentation that makes the
    LSTM less brittle to small input perturbations.
  - Dropout 0.30 -> 0.40, weight_decay 2e-3 -> 5e-3.
  - Drops the aggressive OneCycle (max_lr=3*base) for plain cosine
    annealing -- v9 was overshooting after the warmup spike.
  - Slightly longer window (10 -> 12 days), still within the 5-15d
    sweet spot from the lit review, gives the regularizer a bit more
    signal to chew on without changing regime.

Usage:
    python -m Models.Temporal.lstm.train_v10
"""
...
OUT_DIR  = Path(__file__).parent / "outputs_v10"
...
class BiLSTMAttnReg(nn.Module):
    """BiLSTM + temporal attention pooling with input-noise regularizer."""

    def __init__(self, n_features, hidden_size, num_layers, dropout, proj_size, input_noise):
        super().__init__()
        self.input_noise = input_noise
        self.input_drop  = nn.Dropout(dropout * 0.5)  # mild feature-level dropout
...
def save_loss_curve(train_losses, val_losses):
    try:
        ...
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
        ax.set_title(f"BiLSTM+Attn v10 reg (seq_len={SEQ_LEN})")
        ax.legend()
        fig.tight_layout()
...
def main():
    ...
    print(f"[v10] BiLSTM+Attn+reg  {len(TIME_FEATURES)} time + {len(STATIC_FEATURES)} static  seq_len={SEQ_LEN}  noise={INPUT_NOISE}")
    ...
    print(f"[model] BiLSTMAttnReg  params={n_params:,}")
    ...
    results["config"] = dict(
        variant="v10_bilstm_attn_heavy_reg",
        seq_len=SEQ_LEN, train_stride=TRAIN_STRIDE,
        hidden_size=HIDDEN_SIZE, num_layers=NUM_LAYERS, dropout=DROPOUT,
        proj_size=PROJ_SIZE, input_noise=INPUT_NOISE,
