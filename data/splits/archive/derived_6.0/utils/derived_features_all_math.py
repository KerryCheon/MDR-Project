...
def _smap_cols(imputed: bool = True) -> tuple[str, str]:
    if imputed:
        return "SMAP_sm_am_interp", "SMAP_sm_pm_interp"
    return "SMAP_sm_am", "SMAP_sm_pm"

def _ensure_datetime(df: pd.DataFrame, date_col: str) -> None:
    if date_col in df.columns:
        df[date_col] = pd.to_datetime(df[date_col], errors="coerce")

def add_smap_combined(
    df: pd.DataFrame,
    am_col: str,
    pm_col: str,
    combined_col: str = "SMAP_sm_interp", ) -> pd.DataFrame:

    out = df.copy()
    has_am = am_col in out.columns
    has_pm = pm_col in out.columns

    if has_am and has_pm:
        out[combined_col] = out[am_col].combine_first(out[pm_col])
    elif has_am:
        out[combined_col] = out[am_col]
    elif has_pm:
        out[combined_col] = out[pm_col]
    else:
        # no-op: neither column exists
        return out

    return out

def add_smap_basic_transforms(
    df: pd.DataFrame,
    base_col: str,
    group_col: str = "station_id",
    date_col: str = "date",
    lags: tuple[int, ...] = (1, 7, 30),
    roll_windows: tuple[int, ...] = (7, 30),
    ema_alpha: float = 0.2,) -> pd.DataFrame:
    ...
    # mask for observability
    out[f"{base_col}_mask"] = out[base_col].notna().astype(int)

    # lags
    for k in lags:
        out[f"{base_col}_lag{k}"] = series_lags(
            out, col=base_col, lag_kobs=k, group_col=group_col, date_col=date_col
        )

    # diffs + gradients
    out[f"{base_col}_diff1"] = series_diffs(
        out, col=base_col, kobs=1, group_col=group_col, date_col=date_col
    )
    out[f"{base_col}_grad7"] = series_gradient_kobs(
        out, col=base_col, kobs=7, group_col=group_col, date_col=date_col
    )

    # pct change
    out[f"{base_col}_pctchg"] = series_pct_change(
        out, col=base_col, group_col=group_col, date_col=date_col
    )

    # rolling stats
    for w in roll_windows:
        out[f"{base_col}_rollmean{w}"] = rolling_mean(
            out, col=base_col, window=w, group_col=group_col, date_col=date_col, min_periods=w
        )
        out[f"{base_col}_rollstd{w}"] = rolling_std(
            out, col=base_col, window=w, group_col=group_col, date_col=date_col, min_periods=w
        )
        out[f"{base_col}_rollrange{w}"] = rolling_range(
            out, col=base_col, window=w, group_col=group_col, date_col=date_col, min_periods=w
        )

    # EMA
    out[f"{base_col}_ema{str(ema_alpha).replace('.', '')}"] = ema(
        out, col=base_col, alpha=ema_alpha, group_col=group_col, date_col=date_col
    )

    return out

def add_smap_ampm_diff(
    df: pd.DataFrame,
    am_col: str,
    pm_col: str,
    out_col: str = "SMAP_ampm_diff_interp",
) -> pd.DataFrame:

    out = df.copy()
    if am_col in out.columns and pm_col in out.columns:
        out[out_col] = out[am_col].astype(float) - out[pm_col].astype(float)
    return out

def add_smap_features(
    df: pd.DataFrame,
    group_col: str = "station_id",
    date_col: str = "date",
    imputed: bool = True,
    ...
    roll_windows: tuple[int, ...] = (7, 30),
    ema_alpha: float = 0.2,
    add_ampm_diff: bool = True, ) -> pd.DataFrame:

    out = df.copy()
    _ensure_datetime(out, date_col)

    am_col, pm_col = _smap_cols(imputed=imputed)

    # if neither exists, no-op
    if am_col not in out.columns and pm_col not in out.columns:
        return out

    if make_combined:
        if combined_col is None:
            combined_col = "SMAP_sm_interp" if imputed else "SMAP_sm"
        out = add_smap_combined(out, am_col=am_col, pm_col=pm_col, combined_col=combined_col)

    base_cols: list[str] = []
    if am_col in out.columns:
        base_cols.append(am_col)
    if pm_col in out.columns:
        base_cols.append(pm_col)
    if make_combined and combined_col is not None and combined_col in out.columns:
        base_cols.append(combined_col)

    for base in base_cols:
        out = add_smap_basic_transforms(
            out,
            base_col=base,
            group_col=group_col,
            date_col=date_col,
    ...
    if add_ampm_diff:
        out = add_smap_ampm_diff(out, am_col=am_col, pm_col=pm_col)
