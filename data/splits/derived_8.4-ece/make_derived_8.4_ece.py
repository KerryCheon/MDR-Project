# make_derived_8.4_ece.py
# Compiles derived_8.4-ece split dataset from processed in-situ ECE sensor stations.
# Exact feature parity with derived_8.4 and derived_8.4-oos: 499 standardized columns,
# LIA, static variables, and target > 0.0 filtering.
...
def main():
    ...
    # 4. Load baseline training split for monthly climatology
    ...
    test_df = add_drift_and_anomaly_features(df_static, train_ref_df)
    ...
    if missing_in_ece:
        print(f"[WARN] Filling missing columns with NaN/0 to match exact 499 schema: {missing_in_ece}")
    ...
    meta = {
        ...
        "stations": sorted(list(test_df["station_id"].unique())),
    ...
    print("\nSaved derived_8.4-ece splits successfully:")
    ...
    print(f"  Total: {len(test_df)} rows")
    print(f"  Columns: {len(test_df.columns)} matching derived_8.4 exactly")
