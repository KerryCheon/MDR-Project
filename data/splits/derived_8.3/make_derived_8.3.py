# Compiles derived_8.3 from derived_8.2 by removing Touchet_WA_824, BurntMountain_WA, and HartsPass_WA_515.

import os
import json
import pandas as pd

STATIONS_TO_REMOVE = ["Touchet_WA_824", "BurntMountain_WA", "HartsPass_WA_515"]

def main():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    src_dir = os.path.abspath(os.path.join(script_dir, "..", "derived_8.2"))
    ...
    print("Copying config.yaml (manual comment-out of removed stations)...")
    ...
    meta["source"] = "derived_8.2 splits with Touchet_WA_824, BurntMountain_WA, and HartsPass_WA_515 filtered out"
    meta["rows"] = {
        "train": len(train_filtered),
        "val": len(val_filtered),
        "test": len(test_filtered)
    }
    meta["lia_csv"] = "data/splits/derived_8.3/LIA/stations_lia.csv"
    ...
    print("Dataset compilation for derived_8.3 completed successfully.")
