# make_derived_8.4_oos.py
# Compiles derived_8.4-oos split dataset from processed out-of-state stations.
# Exact feature parity with derived_8.4: 350+ features, LIA, static variables, and target > 0.0 filtering.
...
# Target Year Ranges matching derived_8.4 (2017-2025)
...
def load_processed_stations(config_path: str, project_root: str) -> pd.DataFrame:
    ...
    for key, cfg in stations_cfg.items():
        ...
        # Ensure station_id is standardized string
        if "station_id" in df.columns:
            df["station_id"] = df["station_id"].astype(str)

        unique_ids = df["station_id"].unique()
        if len(unique_ids) > 0 and unique_ids[0] in OOS_STATIONS:
            frames.append(df)
            print(f"  Added station: {unique_ids[0]} ({len(df)} rows)")
        else:
            print(f"  Station {unique_ids} not in OOS list: {OOS_STATIONS}")
...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print("Computing rolling stats, lags, and difference sequences...")
...
def main():
    ...
    config_path = os.path.join(project_root, "src", "pipeline", "config_8.4_oos.yaml")

    print(f"Project root: {project_root}")
    print(f"Loading processed out-of-state stations from config: {config_path}")
    ...
    # Apply target > 0.0 filtering matching derived_8.1_pos / derived_8.4 policy
    initial_cnt = len(combined_df)
    combined_df = combined_df[combined_df[TARGET_COL].notna() & (combined_df[TARGET_COL] > 0.0)].copy()
    print(f"Rows after dropping target <= 0.0 or NaN: {len(combined_df)} (dropped {initial_cnt - len(combined_df)} rows)")
    ...
    meta = {
        "source": "processed station CSVs (Out-of-State evaluated for MDR-Project)",
        ...
        "stations": sorted(list(set(train_df["station_id"].unique()) | set(val_df["station_id"].unique()) | set(test_df["station_id"].unique()))),
        "rows": {
        ...
        "rows_by_station": {
            st: {
                "train": int((train_df["station_id"] == st).sum()),
                "val": int((val_df["station_id"] == st).sum()),
                "test": int((test_df["station_id"] == st).sum()),
                "total": int((train_df["station_id"] == st).sum() + (val_df["station_id"] == st).sum() + (test_df["station_id"] == st).sum())
            } for st in sorted(list(set(train_df["station_id"].unique()) | set(val_df["station_id"].unique()) | set(test_df["station_id"].unique())))
        }
    }
    ...
    print("\nSaved derived_8.4-oos splits successfully:")
