"""Data preprocessing and tensor preparation for derived_8.4-eval-mlp-1.3.

Protocol (data_version 4) — train on the OFFICIAL train split (2017-2020),
early-stop / select configs on the OFFICIAL val split (2021-2022), evaluate on
the untouched test split (2023-2025). Extends mlp-1.1's preprocessing with an
AUXILIARY 2020 holdout: the last year of train (n=2,519) is preprocessed with
the same train-fitted imputer/scaler (no leakage — it is a subset of train) and
evaluated every epoch at the best-val epoch. The aux2020 RMSE is a second,
temporally-adjacent validation signal that exposes val-period overfitting
(mlp-1.1's deep residual nets fit 2021-22 but generalized worst to 2023-25).
