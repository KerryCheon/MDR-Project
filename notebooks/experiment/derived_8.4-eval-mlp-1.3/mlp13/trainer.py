...
def train_one_config(
    ...
    *,
    ...
    stop_rule ("patience" | "plateau" | "val_aux"), mixup_alpha, center_target.

    New in mlp-1.3 (all default to the 1.2 behavior):
      - stop_rule="plateau": stop at the FIRST sustained val plateau (val
        improves by < stop_plateau_eps over stop_plateau_window epochs). 1.2's
        overfitting analysis found test error bottoms out early (~ep 90) while
        val stays flat, so patience-60 picks a late epoch; the plateau rule
        targets the first flat region without touching test.
      - stop_rule="val_aux": best-epoch/patience tracked on (val + aux)/2
        instead of val alone (aux2020 = 2020 slice of train; a train-fit probe).
      - mixup_alpha>0: batch-level mixup (Zhang et al. 2018), lambda ~
        Beta(alpha, alpha) per batch; eval uses the unmixed forward pass.
      - center_target=True: subtract the train mean from all targets (per
        specialist), predict the residual, add the mean back to every recorded
        prediction. RMSE curves stay in original units (shift-invariant).
    """
    ...
    # --- optional target centering (debias at source) -----------------------
    target_shift = 0.0
    if center_target:
    ...
    while not done and epoch < max_epochs:
        ...
        if stop_rule == "plateau" and not retrain_mode:
            # Stop at the FIRST sustained plateau: the best score inside the
            # last `window` epochs must beat the overall best by >= eps, else
            # the curve has flattened. Window bounded by min_epochs = window.
