#!/usr/bin/env python3
"""Run c1 (V6) feature selection on K=2 regime / cluster subsets for eval-3.2.

Uses the recommended pipeline for derived_8.2 from feature-selection-2.1:
  configs/config_c1_baseline_bypass_off.yaml
  (MI → ElasticNet → stability, bypass OFF, top_k=50, stability_n_boot=50)

Also embeds the pre-audited global c1 feature list from feature-selection-2.1
artifacts (does not re-run global selection).

Usage (from repo root, notebooks venv recommended):
  PYTHONPATH=. python notebooks/experiment/derived_8.2-eval-3.2/run_feature_selection.py
  PYTHONPATH=. python notebooks/experiment/derived_8.2-eval-3.2/run_feature_selection.py --n-boot 10  # smoke
"""
...
GLOBAL_C1_ARTIFACT = (
    PROJECT_ROOT
    / "notebooks/experiment/derived_8.2-feature-selection-2.1/artifacts"
    / "derived_8.2/c1_baseline_bypass_off/selected_features.json"
)
...
class QuantileBinner:
    ...
    def predict(self, series: pd.Series) -> np.ndarray:
        ...
        raise NotImplementedError("Only K=2 supported in eval-3.2")
...
def _run_selection_on_subset(
    ...
    cfg_local = json.loads(json.dumps(cfg))  # deep copy via JSON
    cfg_local["models"] = []
    ...
    # Fallback: relax stability min_freq if empty
    if len(feats) == 0:
...
def main(argv=None):
    ...
    if not GLOBAL_C1_ARTIFACT.exists():
        raise FileNotFoundError(f"Missing global c1 artifact: {GLOBAL_C1_ARTIFACT}")

    cfg = _load_config(CONFIG_PATH)
    global_c1_payload = json.loads(GLOBAL_C1_ARTIFACT.read_text(encoding="utf-8"))
    global_c1_feats = list(global_c1_payload["features"])

    train = _add_temporal(pd.read_csv(PROJECT_ROOT / "data/splits/derived_8.2/train.csv"))
    val = _add_temporal(pd.read_csv(PROJECT_ROOT / "data/splits/derived_8.2/val.csv"))
    test = _add_temporal(pd.read_csv(PROJECT_ROOT / "data/splits/derived_8.2/test.csv"))

    drop_cols = list(cfg.get("data", {}).get("id_cols", ["station_id"])) + [
        cfg.get("data", {}).get("time_col", "date")
    ]
    # month/year are engineered after load; drop if present so preprocess does not
    # treat them as candidate predictors inconsistently across subsets
    ...
    # Align indices for mask application
    ...
    # Resolve labels for non-binary strategies from original frames (need G_API etc.)
    ...
    km = KMeansClusterer(COLS_DYNAMIC, 2)
    km.fit(train)
    strategies["Clustering_Dynamic_k2"]["labels"] = {
        "train": km.predict(train.loc[X_tr.index]),
        "val": km.predict(val.loc[X_va.index]),
        "test": km.predict(test.loc[X_te.index]),
    }
        ...
        for c in range(K):
            ...
            if n_tr < args.min_train:
                print(
                    ...
                    f"using global c1 as fallback for {meta_key}"
                )
                feats = list(global_c1_feats)
                selection_meta[meta_key] = {
                    "n_train": n_tr,
                    "n_val": n_va,
                    "n_test": n_te,
                    "status": "fallback_global_c1_small_n",
                    "n_features": len(feats),
                }
            else:
                run_dir = ARTIFACTS_DIR / strat_name / f"c{c}" / "run"
                run_id = f"eval32_{strat_name}_c{c}"
                ...
                status = "ok"
                if len(feats) == 0:
                    print(f"[WARNING] empty selection for {meta_key}; fallback global c1")
                    feats = list(global_c1_feats)
                    status = "fallback_global_c1_empty"
                selection_meta[meta_key] = {
                    "n_train": n_tr,
                    "n_val": n_va,
                    "n_test": n_te,
...
def _build_payload(
    ...
    return {
        ...
        "global_c1": {
            "n": len(global_c1_feats),
            "features": list(global_c1_feats),
            "source": str(GLOBAL_C1_ARTIFACT.relative_to(PROJECT_ROOT)),
            "artifact_created": global_c1_payload.get("created"),
            "artifact_git_commit": global_c1_payload.get("git_commit"),
        },
