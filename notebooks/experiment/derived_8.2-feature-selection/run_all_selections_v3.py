...
def main():
    ...
    v3_str = f"OVERALL_SELECTED_FEATURES_V3 = {repr(list(selected_feats))}"
    
    # Process file lines to update or append OVERALL_SELECTED_FEATURES_V3
    lines = content.splitlines()
    new_lines = []
    has_v3 = False
    for line in lines:
        if line.startswith("OVERALL_SELECTED_FEATURES_V3"):
            new_lines.append(v3_str)
            has_v3 = True
        else:
    ...
    if not has_v3:
        new_lines.append("")
        new_lines.append("# Selected features for overall global model - V3 (expanded feature set)")
        new_lines.append(v3_str)
    ...
    print(f"Successfully updated {dest_path} with OVERALL_SELECTED_FEATURES_V3!")
