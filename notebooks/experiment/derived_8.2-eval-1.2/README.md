# derived_8.2-eval-1.2 (Global Models Comparison Report)

This directory contains the training and evaluation notebook for comparing five **single global** XGBoost models on the Washington-only `derived_8.2` dataset.
...
4. **Model V4**: Trained using **OVERALL_SELECTED_FEATURES_V4** (50 features, no MI stage, min_freq=0.01).
5. **Model V5**: Trained using **OVERALL_SELECTED_FEATURES_V5** (32 features, no MI stage, min_freq=0.6).
...
| **Model V5 (32 Features, Weighted, $\beta=0.2$)** | 0.5948 | 0.0670 | 0.0657 | −0.0133 | 0.0499 | 0.0386 | 0.7870 |
...
### 2. Restoring `min_freq: 0.6` without MI (Model V5) further degrades performance
Model V5, which skips the MI stage and filters features strictly with `min_freq: 0.6`, performs even worse than Model V4 ($R^2$ of **0.5957** vs. **0.6088**).
- **Collinearity and L1 regularisation**: Without the pre-filtering of the Mutual Information stage, the ElasticNet selector is confused by the high collinearity of the full feature space. This results in a very sparse selection (only 34 non-zero coefficients), which stability bootstrap further trims to 32 stable features.
- **MI Stage is Indispensable**: The MI stage performs a critical pruning of weakly-related features. By reducing the candidate space from 400+ to 300 features, it resolves collinearity confusion, allowing the stability selection to identify a highly predictive, stable set of features (V3) that yields a $+0.0517$ absolute $R^2$ improvement over the no-MI equivalent (V5).
...
| **Model V5 (32 Features, Weighted, $\beta=0.2$)** | 0.5752 | 0.0701 | 0.0648 | −0.0268 | 0.0518 | 0.0406 | 0.8035 |
...
| **Model V5 (32 Features, Weighted, $\beta=0.2$)** | 0.5704 | 0.0654 | 0.0643 | −0.0123 | 0.0482 | 0.0374 | 0.7772 |
...
| **Model V5 (32 Features, Weighted, $\beta=0.2$)** | 0.6167 | 0.0648 | 0.0648 | +0.0031 | 0.0495 | 0.0376 | 0.7871 |
...
- **Model V5** performs poorly across all three test years, further verifying that the selected features are less expressive without the pre-pruning benefits of the MI stage.
