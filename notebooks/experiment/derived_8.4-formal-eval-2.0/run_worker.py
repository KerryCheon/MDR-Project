#!/usr/bin/env python3
"""Train or evaluate a single derived_8.4-formal-eval-2.0 job: (config_id, seed, target).

target:
  - 'full': Temporal evaluation on Washington test set (2023-2025, 7 stations).
  - 'oos': Spatial evaluation on derived_8.4-oos (2017-2025, 10 out-of-state stations).

Usage:
    python run_worker.py --config-id <id> --seed <s> --target <full|oos> \
...
def main() -> None:
    ...
    parser.add_argument("--target", required=True, choices=["full", "oos"],
                        help="'full' for WA temporal or 'oos' for Out-of-State spatial")
    ...
    result = evaluator.evaluate_full() if is_full else evaluator.evaluate_spatial_oos()
