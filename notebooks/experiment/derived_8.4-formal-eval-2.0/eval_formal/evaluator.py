"""Seed-aware model evaluation for derived_8.4-formal-eval-2.0.

Evaluates:
1. Full (temporal) evaluation on derived_8.4 test set (2023-2025, 7 WA stations).
2. Out-of-State (OOS) spatial evaluation on derived_8.4-oos (2017-2025, 10 unseen stations:
...
The out-of-state dataset is completely unseen during training.
"""
...
class FormalEvaluator:
    """Evaluates one fixed configuration under temporal (WA test) or spatial (OOS) evaluation."""
    ...
    def evaluate_spatial_oos(self) -> SpatialEvalResult:
        """Spatial evaluation on derived_8.4-oos: models/routers fit on WA trainval,
        evaluated on all 10 out-of-state stations (2017-2025, 25,176 rows)."""
        trainval = self.data.trainval
        oos_data = self.data.oos_all
        if oos_data.empty:
            raise ValueError("No out-of-state data found in ExperimentData.oos_all")

        y_trainval = trainval[self.data.target].to_numpy(dtype=float)
        y_oos = oos_data[self.data.target].to_numpy(dtype=float)

        router = self._router()
        router.fit(trainval)
        labels_trainval = np.asarray(router.predict(trainval)).ravel().astype(int)
        labels_oos = np.asarray(router.predict(oos_data)).ravel().astype(int)

        predictions = np.zeros(len(oos_data), dtype=float)
        cluster_metrics: dict[str, dict[str, float]] = {}
        params = self._params()
        started = perf_counter()
        fitted_models: dict[str, Any] = {}
        ...
        # Check if model checkpoints already exist
        ...
        for cluster in clusters_to_eval:
            ...
            if expert is None:
                if not train_mask.any():
                    predictions[oos_mask] = float(np.mean(y_trainval))
                    cluster_metrics[cluster_key] = {
                        "n_train": 0.0,
                        "n_test": float(oos_mask.sum()),
                        **compute_metrics(y_oos[oos_mask], predictions[oos_mask]),
                    }
                    continue
            ...
            if oos_mask.any():
                ...
                pred = np.asarray(
                    expert.predict(oos_data.loc[oos_mask, booster_feats])
                ).ravel()
                predictions[oos_mask] = pred
                cluster_metrics[cluster_key] = {
                    "n_train": float(train_mask.sum()),
                    "n_test": float(oos_mask.sum()),
                    **compute_metrics(y_oos[oos_mask], pred),
                }
            else:
                cluster_metrics[cluster_key] = {
                    "n_train": float(train_mask.sum()),
        ...
        pooled = compute_metrics(y_oos, predictions)
        pooled["n"] = int(len(y_oos))

        station_ids = oos_data["station_id"].to_numpy()
        station_metrics: dict[str, dict[str, float]] = {}
        for s in sorted(oos_data["station_id"].unique()):
            mask = station_ids == s
            m = compute_metrics(y_oos[mask], predictions[mask])
            m["n"] = int(mask.sum())
            station_metrics[str(s)] = m

        yearly_metrics: dict[str, dict[str, float]] = {}
        for year in sorted(oos_data["year"].unique()):
            mask = oos_data["year"].to_numpy() == year
            yearly_metrics[str(int(year))] = compute_metrics(y_oos[mask], predictions[mask])

        train_time = perf_counter() - started

        stem = self._stem("oos")
        self._persist(stem, predictions, labels_oos, pooled, train_time,
                      n_train_total=int(len(trainval)),
                      router=router, fitted_models=fitted_models)
        ...
        return SpatialEvalResult(
            ...
            train_time_s=train_time,
            predictions=predictions,
            cluster_labels_test=labels_oos,
        )
