"""Figures for derived_8.4-formal-eval-2.0 (generated by the report notebook).
...
- spatial: per-station pair scatter (A vs B, identity line, "k of 10 stations" win
  annotation), per-station bars with seed error bars for 10 OOS stations, and
...
def plot_seed_boxplot(seed_df: pd.DataFrame, cfg_frame: pd.DataFrame, out_dir: Path,
    ...
    title_target = "temporal test set" if prefix == "temporal" else "10 unseen out-of-state stations"
...
def plot_delta_robustness(temp_df: pd.DataFrame, spat_df: pd.DataFrame,
    ...
    ax2.set_ylabel(f"Spatial (OOS) {metric.upper()} (mean ± std over seeds)")
    ax2.set_title(f"Spatial (OOS) Delta-Source Robustness ({metric.upper()})")
...
def plot_spatial_pair(per_station_df: pd.DataFrame, cfg_frame: pd.DataFrame, out_dir: Path,
    ...
    """Spatial (OOS) per-station pair scatter: A vs B (per-station median over seeds),
    with identity line and 'A wins k of 10 stations' annotation."""
    a, b = pair
    med = (
    ...
    ax.set_title(f"OOS Spatial per-station {metric.upper()}: {_label(a, cfg_frame)} wins {wins} of {n} stations")
    fig.tight_layout()
...
def plot_spatial_station_bars(per_station_df: pd.DataFrame, cfg_frame: pd.DataFrame,
    ...
    """Per-station OOS metric with seed error bars for one config."""
    ...
    ax.set_ylabel(f"OOS Spatial {metric.upper()}")
    ax.set_title(f"OOS per-station {metric.upper()} (mean ± std over seeds): {_label(config_id, cfg_frame)}")
    fig.tight_layout()
