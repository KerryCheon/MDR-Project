"""Configuration pinning for derived_8.4-formal-eval-1.0.

Builds the 20 pinned configurations:
  - 14 requested configs with test-selected additions (parsed from
    derived_8.4-eval-1.1/delta_grid_summary.csv, identical parsing to eval-1.3) or none;
  - 6 val-selected winners (read from val_selected_deltas.json, written by
    select_deltas_val.py — run it first).
...
def load_pinned_configs(data, config: dict, require_val: bool = True) -> list[dict]:
    ...
    # Val-selected winners (6): one per MoE strategy, from select_deltas_val.py.
    ...
    for strategy in config["val_selection"]["strategies"]:
        ...
        configs.append({
            ...
            "cluster_additions": {
                # Slice the ranked additions to the val-winning counts.
