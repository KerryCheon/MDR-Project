...
from lstm.train import (
    train_v23_and_extract_all, compute_pca_all, extract_all_representations,
    load_best_model, TOP25_FEATURES, SEQ_LEN, PCA_LEVELS,
)
...
def main():
    print("=" * 75, flush=True)
    print("Starting derived_8.4-hybrid-lstm-1.4 Evaluation (V23 + PCA)", flush=True)
    print("=" * 75, flush=True)
    ...
    # Phase 1-3: Train V23, extract representations, compute PCA
    ...
    if has_raw and has_pca:
        print("[V23] Found existing raw + PCA representations. Skipping training.", flush=True)
    else:
        if has_raw and not has_pca:
            print("[V23] Raw found, PCA missing. Recomputing PCA only.", flush=True)
            reps_map = {
    ...
    lstm_record = {
        ...
        "global_feature_count": len(TOP25_FEATURES),
    ...
    all_metrics["BiLSTM+Attn (LSTM-only, V23)"] = lstm_record
    ...
    print("\n" + "=" * 75, flush=True)
    print("FINAL MODEL LEADERBOARD (derived_8.4-hybrid-lstm-1.4)", flush=True)
    print("=" * 75, flush=True)
...
def generate_readme(df_summary: pd.DataFrame, df_regime: pd.DataFrame, shap_info: dict):
    ...
    readme_content = f"""# Experiment: `derived_8.4-hybrid-lstm-1.4` — V23 LSTM + PCA + XGBoost with Accelerated SHAP
