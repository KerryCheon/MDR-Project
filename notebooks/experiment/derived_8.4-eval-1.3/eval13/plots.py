"""LOSO visualization utilities for derived_8.4-eval-1.3."""
...
STRATEGY_COLORS = {
    ...
    "Clustering_Backbone54_k2": "#17becf",  # NEW: sibling of V0_Full, routes on the 54 backbone
...
def plot_full_vs_loso_bars(df_merge: pd.DataFrame, out_dir: Path) -> Path:
    ...
    ax.set_ylabel("Median R² (over 47 configs)")
...
def plot_loso_gap_boxplot(df_full_pcs: pd.DataFrame, df_loso_pcs: pd.DataFrame, out_dir: Path) -> Path:
    """Per-station distribution of (full-training R2 − LOSO R2) over the 47 configs.
    ...
    ax.set_title("LOSO cost per station — full-training R² minus LOSO R² over 47 configs",
                 fontsize=12, weight="bold")
    ax.grid(True, axis="y", ls="--", alpha=0.3)
    plt.tight_layout()
