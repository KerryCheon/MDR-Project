"""Routing / Gating strategy implementations for derived_8.4-eval-1.3 (LOSO).

Same 6 routers as derived_8.4-eval-1.1 / eval-1.2, plus the NEW
`Clustering_Backbone54_k2` strategy: a KMeans(k=2) router fitted on the same
54 shared-backbone features as the single-regime global model, so the two-regime
model is presented as developed from the single-regime model (no separate V0-50
feature source to explain). Every router exposes a `fit(train)` method so it can
be refitted per LOSO fold on the fold's trainval only (no held-out-station
leakage into routing).
"""
...
class Backbone54Router:
    """KMeans(k=2) router fitted on the 54 shared-backbone features.
    ...
    two-regime model is a direct development of the single-regime model —
    identical structure to ``V0FullRouter``, only the feature set differs.
    """
    ...
    def predict(self, frame: pd.DataFrame) -> np.ndarray:
        if self.means is None:
            raise RuntimeError("Backbone54Router must be fitted before predict.")
        values = frame.loc[:, self.features].copy().fillna(self.means)
        return self.kmeans.predict(self.scaler.transform(values))
...
def get_router(strategy_name: str, v0_features: list[str], backbone_54: list[str] | None = None,
    ...
    if strategy_name == "Global_Single":
        ...
        return Backbone54Router(backbone_54 or [], seed=seed)
