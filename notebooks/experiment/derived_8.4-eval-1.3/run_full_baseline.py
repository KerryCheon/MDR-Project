#!/usr/bin/env python3
"""Full-training baseline for derived_8.4-eval-1.3 (replicates derived_8.4-eval-1.1).
...
a station suffers when held out). The pooled test R2 of the NEW Clustering_Backbone54_k2
configs (no eval-1.1 row) is their temporal reference, merged into the LOSO summary by
run_loso.py.
...
Usage:
    python run_full_baseline.py                     # full run (56 configs)
...
def main() -> None:
    ...
    print("=" * 70, flush=True)
    print("Starting derived_8.4-eval-1.3 Full-Training Baseline (replicates eval-1.1)", flush=True)
    print("=" * 70, flush=True)
