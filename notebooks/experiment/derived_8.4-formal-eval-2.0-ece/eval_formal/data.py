"""Data loading for derived_8.4-formal-eval-2.0-ece.

Loads:
1. derived_8.4 splits (7 Washington stations: train 2017-2020, val 2021-2022, test 2023-2025).
2. derived_8.4-ece splits (5 in-situ ECE stations in WA: ECE_BBG_Main_St, ECE_BBG_Lost_Meadow,
...
def load_experiment_data(project_root: Path, config: dict[str, Any]) -> ExperimentData:
    ...
    # 2. In-situ ECE split (derived_8.4-ece)
    ...
    if "splits" in ece_cfg:
        ...
        ece_train = frames_ece["train"]
        ece_val = frames_ece["val"]
        ece_test = frames_ece["test"]
        ece_all = pd.concat([ece_train, ece_val, ece_test], axis=0, ignore_index=True)
    else:
        ece_train = pd.DataFrame()
        ece_val = pd.DataFrame()
        ece_test = pd.DataFrame()
