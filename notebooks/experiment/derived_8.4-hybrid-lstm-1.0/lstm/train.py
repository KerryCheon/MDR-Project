"""
LSTM Training and Frozen Context Vector (CTX) Extraction Module.

Phase 1: Trains BiLSTMAttn model on derived_8.4 sequence dataset until convergence.
Phase 2: Freezes best_model.pt (eval mode, torch.no_grad()) and extracts 160-dim ctx vectors for train, val, test splits.
"""
...
def train_lstm_and_extract_ctx(data_dir: Path, artifacts_dir: Path):
    ...
    # Phase 2: Load frozen converged model & extract ctx vectors
    print("[Phase 2: Frozen CTX Extraction] Loading best checkpoint...", flush=True)
