#!/usr/bin/env python3
"""Run c1 (V6) feature selection on K=2 regime / cluster subsets for derived_8.4-eval-1.0.

Uses the recommended pipeline for derived_8.4 from feature-selection-1.0:
  configs/config_c1_baseline_bypass_off.yaml
  (MI → ElasticNet → stability, bypass OFF, top_k=50, stability_n_boot=50)

Embeds the OVERALL_SELECTED_FEATURES_V0 list from data/splits/derived_8.4/dataset_metadata.py
...
def main(argv=None):
    ...
    train = _add_temporal(pd.read_csv(PROJECT_ROOT / "data/splits/derived_8.4/train.csv"))
    val = _add_temporal(pd.read_csv(PROJECT_ROOT / "data/splits/derived_8.4/val.csv"))
    test = _add_temporal(pd.read_csv(PROJECT_ROOT / "data/splits/derived_8.4/test.csv"))
    ...
    for strat_name, strat in strategies.items():
        ...
        for c in range(K):
            ...
            if n_tr < args.min_train:
                ...
                run_id = f"eval84_{strat_name}_c{c}"
...
def _build_payload(
    ...
    return {
        ...
        "global_c1": {
            ...
            "source": "data/splits/derived_8.4/dataset_metadata.py::OVERALL_SELECTED_FEATURES_V0",
        },
        "global_v0": {
            "n": len(global_v0_feats),
            "features": list(global_v0_feats),
