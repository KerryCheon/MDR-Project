"""Guarded-on-ECE evidence runner for paper 2 final evidence 1.0.
...
Protocol (ECE strictly unseen):
  ...
  - Historical policy names map semantically per family: c0_only is the dry
    manual oracle, and c1_only is the wet-expert diagnostic. Guarded's local
    c1 is dry, so its expert indices intentionally flip relative to salvage.
...
def calibrate_wa(train: pd.DataFrame, val: pd.DataFrame, routers: dict,
                 ...
                 params: dict, expert_indices: dict[str, dict]) -> dict:
    ...
    for family, key in families.items():
        ...
        aux_semantic = np.asarray(routers["gapi"].predict(val)).ravel().astype(int)
        aux = np.where(aux_semantic == 0, semantic["dry_expert_index"],
                       semantic["wet_expert_index"])
        ...
        aux_m_semantic = np.asarray(routers["gapi"].predict(masked)).ravel().astype(int)
        aux_m = np.where(aux_m_semantic == 0, semantic["dry_expert_index"],
                         semantic["wet_expert_index"])
        for name, preds in [
...
def run_seed(seed: int, data: dict, routers: dict, v0_features: list[str],
    ...
    for family, key in families.items():
        ...
        aux_labels = np.where(aux_semantic == 0, dry_idx, wet_idx)
...
def main(argv: list[str] | None = None) -> None:
    ...
    calibration = calibrate_wa(data["train"], data["val"], routers, v0_features,
                               backbone, config, params, expert_indices)
