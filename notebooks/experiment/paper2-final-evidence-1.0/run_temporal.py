#!/usr/bin/env python3
"""Temporal evaluation driver for derived_8.4-routing-optimize-1.0 (frozen split, 30 seeds).

Port of derived_8.4-formal-eval-1.0/run_temporal.py: pins the no-delta
configurations, spawns one run_worker.py job per (config, seed) on the GPU
("full" station = temporal full-baseline protocol: experts on trainval, test on
...
Usage:
    python run_temporal.py                                  # full run (3 configs x 30 seeds)
    python run_temporal.py --smoke --max-configs 1 --seeds 42  # CPU smoke
"""
...
from train_ev.configs import config_frame, load_pinned_configs  # noqa: E402
...
def main() -> None:
    ...
    print("=" * 70, flush=True)
    print("Temporal evaluation (frozen split, multi-seed) — derived_8.4-routing-optimize-1.0", flush=True)
    print("=" * 70, flush=True)
    ...
    # Replication anchor: seed-42 Backbone54 (0,0) pooled R2 must match the
    # eval-1.3 full baseline (identical router on the full trainval + identical
    # expert hyperparameters). Guarded/StationMean must match it too (same
    # full-trainval partition, §R2 ARI=1.0).
    print("\nSEED-42 REPLICATION CHECK (pooled test R2)", flush=True)
    if args.smoke:
