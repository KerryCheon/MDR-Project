#!/usr/bin/env python3
"""Train a single derived_8.4-routing-optimize-1.0 job: one (config_id, seed[, station]).

Port of derived_8.4-formal-eval-1.0/run_worker.py: the drivers pin the
configurations, write artifacts/runtime.json {data_version, device,
n_estimators}, and spawn these workers as subprocesses. Each worker loads the
derived_8.4 split, trains the configuration's experts with the job's seed
(router stays at the fixed config router_seed), evaluates, and persists:
    ...
    models/<config_id>__s<seed>__<station>_{meta,spec_0,spec_1}.json
...
from train_ev.evaluator import RoutingOptEvaluator  # noqa: E402
