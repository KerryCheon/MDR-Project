...
def main() -> None:
    ...
    for fname, frame in frames.items():
        ...
        for name in names:
            ...
            summary_rows.append({"split": fname, "router": name, "n_rows": len(frame),
                                 "cluster_0_rows": sizes[0] if sizes else 0,
                                 "cluster_1_rows": sizes[1] if len(sizes) > 1 else 0,
                                 "mean_station_purity": float(pur)})
    ...
    print()
    pd.DataFrame(summary_rows).to_csv(EXP_DIR / "agreement_summary.csv", index=False)
    pd.DataFrame(pairwise_rows).to_csv(EXP_DIR / "agreement_pairwise.csv", index=False)
    pd.DataFrame(fold_rows).to_csv(EXP_DIR / "loso_fold_agreement.csv", index=False)

    print("### K-sweep quality (Backbone54, trainval-only)")
    k_sweep = k_sweep_quality(data.trainval, b54, tuple(config["k_sweep"]), seed)
    print(k_sweep.to_markdown(index=False))
    k_sweep.to_csv(EXP_DIR / "k_sweep_quality.csv", index=False)
    print("[Artifacts] Wrote agreement_summary.csv, agreement_pairwise.csv, "
          "loso_fold_agreement.csv, and k_sweep_quality.csv")
