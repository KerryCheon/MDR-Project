"""
LSTM Training and Frozen Representation Extraction Module (hidden-size sweep).
...
This experiment intentionally does NOT apply PCA: the hypothesis (vs
derived_8.4-hybrid-lstm-1.5, which used H=80 + PCA) is that a smaller hidden
size yields a compact representation that does not dilute the 54 tabular
features in the downstream hybrid XGBoost models.
"""
...
def train_lstm_hidden(hidden_size: int, data_dir: Path, models_dir: Path):
    ...
    print(f"[V21 H{hidden_size}] {len(ALL_FEATURES)} features, seq_len={SEQ_LEN}, {len(SEEDS)} seeds", flush=True)
    train_df = pd.read_csv(data_dir / "train.csv")
    imputer, scaler = fit_preprocessors(train_df, ALL_FEATURES)

    results = []
    for seed in SEEDS:
        val_rmse = train_one_seed(seed, ALL_FEATURES, SEQ_LEN, data_dir, models_dir, (imputer, scaler), hidden_size)
...
def train_lstm_for_hidden(hidden_size: int, data_dir: Path, artifacts_dir: Path, models_dir: Path):
    ...
    model = load_best_model(checkpoint_path, len(ALL_FEATURES), device, hidden_size)

    print("\n--- LSTM-only evaluation ---", flush=True)
    lstm_metrics = compute_lstm_only_metrics(model, data_dir, artifacts_dir, ALL_FEATURES, SEQ_LEN)

    print("\n--- Representation extraction ---", flush=True)
    reps_map = extract_all_representations(model, data_dir, artifacts_dir, ALL_FEATURES, SEQ_LEN)
