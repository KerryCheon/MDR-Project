...
class HybridStrategyEvaluator:
    ...
    def fit_and_evaluate(
        ...
        for cluster in unique_clusters:
            ...
            booster_file = self.models_dir / f"{candidate_id}_cluster_{cluster}.json" if self.models_dir else None

            if booster_file and booster_file.exists():
                model = XGBRegressor(**self._params())
                model.load_model(str(booster_file))
                import torch
                if torch.cuda.is_available():
                    model.get_booster().set_param({"device": "cuda", "tree_method": "hist"})
            else:
                model = XGBRegressor(**self._params())
