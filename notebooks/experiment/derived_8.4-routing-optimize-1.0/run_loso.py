#!/usr/bin/env python3
"""LOSO spatial evaluation driver for derived_8.4-routing-optimize-1.0 (5 seeds).

Port of derived_8.4-formal-eval-1.0/run_loso.py: same pinned no-delta
configurations; for each held-out station the router is refitted on the
6-station trainval (no held-out-station leakage into routing) and experts are
trained per regime cluster, evaluated on the held-out station's 2023-2025 test
rows. Spawns one run_worker.py job per (config, seed, station), then aggregates
per-job meta.json into:
...
Usage:
    python run_loso.py                                    # full run (3 configs x 5 seeds x 7 stations)
    python run_loso.py --smoke --max-configs 1 --seeds 42 --max-stations 1
"""
...
from train_ev.configs import load_pinned_configs  # noqa: E402
from routing_opt.data import load_experiment_data  # noqa: E402
from train_ev.jobs import (  # noqa: E402
    expected_version,
    job_complete,
    load_job_meta,
    make_jobs,
...
def main() -> None:
    ...
    print("=" * 70, flush=True)
    print("LOSO spatial evaluation (multi-seed) — derived_8.4-routing-optimize-1.0", flush=True)
    print("=" * 70, flush=True)
    ...
    # Replication anchor: seed-42 Backbone54 (0,0) LOSO mean R2 must match
    # eval-1.3/eval-1.4 (0.6171). Canonicalization only permutes labels globally
    # per fold, which pooled metrics are invariant to, so |diff| < 1e-3 is expected.
    print("\nSEED-42 REPLICATION CHECK (LOSO mean R2)", flush=True)
    if args.smoke:
