"""Shared parallel-job machinery for derived_8.4-routing-optimize-1.0 training.

Verbatim port of derived_8.4-formal-eval-1.0/eval_formal/jobs.py paths and resume
semantics (ARTIFACTS_DIR/EXP_DIR resolve from this file's location, so no edits
were needed beyond the module move): the driver pins configurations, writes
artifacts/runtime.json, and spawns n_parallel worker subprocesses — each trains
one (config, seed[, station]) job — then aggregates per-job meta.json files.
"""
