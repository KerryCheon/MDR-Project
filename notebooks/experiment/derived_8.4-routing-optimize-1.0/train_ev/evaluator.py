"""Seed-aware fold/model evaluation for derived_8.4-routing-optimize-1.0.

Faithful port of derived_8.4-formal-eval-1.0/eval_formal/evaluator.py with one
change: routers come from routing_opt (guarded backbone family + references)
instead of eval_formal.routers. Protocol is otherwise identical: XGBoost expert
regressors use the per-job seed, the router stays at the fixed config
router_seed, artifacts embed <config_id>__s<seed>__<station>.
"""
...
from routing_opt.data import ExperimentData
...
class RoutingOptEvaluator:
    ...
    def _router(self):
        return get_router(
            ...
            v0_features=self.data.v0_features,
            seed=self.router_seed,
            tau=float(self.config["router"].get("tau", 0.10)),
        )
    ...
    def _persist(self, stem: str, predictions: np.ndarray, labels_test: np.ndarray,
        ...
        """Save predictions, labels, model weights and rely on the worker for meta.json."""
