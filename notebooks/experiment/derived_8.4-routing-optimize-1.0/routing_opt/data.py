"""Data loading for derived_8.4-routing-optimize-1.0.

Same split semantics as derived_8.4-eval-1.4/eval14/data.py: loads the frozen
derived_8.4 temporal split, builds trainval = train + val, exposes the shared
54-feature backbone (pinned in config.yaml) and the legacy V0-50 list (from
data/splits/derived_8.4/dataset_metadata.py, reference only).
"""
...
def load_experiment_data(project_root: Path, config: dict[str, Any]) -> ExperimentData:
    ...
    missing = [c for c in shared_backbone if c not in train.columns]
    if missing:
        raise ValueError(f"Backbone features missing from split: {missing}")
