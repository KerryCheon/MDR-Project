...
def load_config(path: str | Path) -> dict[str, Any]:
    ...
    if forbidden:
        raise ValueError(
            "The local 1.1 selector forbids temporal weighting or drift "
    ...
    delta_counts = tuple(
        int(value) for value in config["search"].get("delta_addition_counts", [0])
    )
    if delta_counts != (0,):
        raise ValueError(
            "The 1.1 selector is intentionally global-only: "
            f"delta_addition_counts must be [0], got {delta_counts}."
        )
    minimum = int(config["search"]["global_feature_min"])
    maximum = int(config["search"]["global_feature_max"])
    if minimum < 1 or minimum > maximum:
        raise ValueError(
            "The selector requires a positive global feature range with min <= max."
        )
    sizes = tuple(
        int(value) for value in config["search"].get("normalization_sizes", ())
    )
    if not sizes or tuple(sorted(set(sizes))) != sizes:
        raise ValueError("normalization_sizes must be a non-empty sorted unique list.")
    if any(size < minimum or size > maximum for size in sizes):
        raise ValueError(
            "Every normalization size must be inside the configured global feature range."
        )
    largest_delta = max(delta_counts)
    required_pool_size = maximum + largest_delta
