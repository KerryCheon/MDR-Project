...
class SearchRecorder:
    ...
    def invalidate_final_outputs(self) -> None:
        ...
        for path in self.artifact_dir.glob("selected_features_*.json"):
            path.unlink()
        by_size_path = self.artifact_dir / "selected_features_by_size.json"
        if by_size_path.exists():
            by_size_path.unlink()
...
class DirectSearch:
    ...
    def __init__(
        ...
        *,
        ...
        if int(config["search"]["candidate_pool_size"]) < minimum_pool_size:
            raise ValueError(
                "candidate_pool_size must leave room for the largest specialist "
    ...
    def _cluster_external_ranking(
        ...
        if max(DELTA_ADDITION_COUNTS) == 0:
            return []
    ...
    def _validate_calibration(self, baseline: CandidateResult) -> None:
        ...
        if not bool(calibration.get("validate", True)):
            print(
                "[search] calibration parity check skipped: the original baseline "
                "uses SMAP features removed from this local selector",
                flush=True,
            )
            return
    ...
    def _summary(
        ...
        *,
        ...
        selected_by_size: dict[str, dict[str, Any]] | None = None,
        ...
        if selected_by_size is not None:
            summary["selected_feature_sizes"] = selected_by_size
    ...
    def run(self) -> dict[str, Any]:
        """Run calibration, wrapper rounds, then the required no-delta final grid."""
        ...
        minimum = int(self.config["search"]["global_feature_min"])
        maximum = int(self.config["search"]["global_feature_max"])
        viable = [
            result
            for result in parent_results
            if minimum <= len(result.global_features) <= maximum
        ]
        parents = sorted(viable or parent_results, key=self._rank_key)[:3]

        normalized_candidates: list[Candidate] = []
        for parent in parents:
            for target_size in self.config["search"].get("normalization_sizes", [minimum, maximum]):
        ...
        current_candidates = self._distinct_results(
            [baseline, *seed_results, *normalized_batch.resolved.values()]
        )
        if not current_candidates:
            self._fail_incomplete(
                "no complete global candidate completed",
                baseline=baseline,
                current=None,
                candidate_pool=candidate_pool,
                rounds=[],
        ...
        current = self.best(current_candidates)
        ...
        print("[search] evaluating the required no-delta final grid", flush=True)
        ...
        if any(row["evaluation_status"] == "missing" for row in delta_rows):
            self._fail_incomplete(
                "the required no-delta final grid did not complete before the deadline",
                baseline=baseline,
                current=current,
                candidate_pool=candidate_pool,
                rounds=rounds,
        ...
        exact_results = [
            result
            for result in self.recorder.results
            if result.model_kind == "exact"
            and not result.cluster_additions.get("0")
            and not result.cluster_additions.get("1")
        ]
        if not exact_results:
            self._fail_incomplete(
                "no complete exact no-delta candidate was retained",
                baseline=baseline,
                current=current,
                candidate_pool=candidate_pool,
                rounds=rounds,
        ...
        # The wrapper chooses one evidence-ranked base candidate.  The published
        # feature-size variants are deterministic nested normalizations of that
        # same candidate, rather than four independent searches.
        selected_sizes = [
            int(size)
            for size in self.config["search"].get(
                "normalization_sizes", [minimum, maximum]
            )
        ]
        normalized_size_candidates = [
            Candidate(
                f"selected_size_{size}",
                tuple(self._normalize(winner.global_features, size, candidate_pool)),
            )
            for size in selected_sizes
        ]
        size_batch = self._evaluate_many(
            normalized_size_candidates,
            "exact",
            phase="selected_size_grid",
            launch_deadline=self.deadline,
            include_predictions=True,
        )
        selected_by_size: dict[str, dict[str, Any]] = {}
        previous: set[str] | None = None
        for size, candidate in zip(selected_sizes, normalized_size_candidates):
            features = list(candidate.global_features)
            if len(features) != size:
                self._fail_incomplete(
                    f"selected size {size} normalized to {len(features)} features",
                    baseline=baseline,
                    current=current,
                    candidate_pool=candidate_pool,
                    rounds=rounds,
            ...
            if previous is not None and not previous.issubset(feature_set):
                self._fail_incomplete(
                    "selected feature-size manifests are not nested",
                    baseline=baseline,
                    current=current,
                    candidate_pool=candidate_pool,
                    rounds=rounds,
            ...
            result = size_batch.resolved.get(candidate.candidate_id)
            if result is None:
                self._fail_incomplete(
                    f"selected size {size} was not evaluated before the deadline",
                    baseline=baseline,
                    current=current,
                    candidate_pool=candidate_pool,
                    rounds=rounds,
            ...
            selected_by_size[str(size)] = {
                ...
                "pooled_r2": result.pooled_r2 if result is not None else None,
                "pooled_rmse": result.pooled_rmse if result is not None else None,
                "pooled_mae": result.pooled_mae if result is not None else None,
            }
        summary = self._summary(
            status="complete",
            baseline=baseline,
        ...
        self.recorder.write_json(
            "selected_features_by_size.json",
            {
                "status": "complete",
                "selection_method": "nested evidence-ranked normalization of one wrapper winner",
                "candidate_pool_size": len(candidate_pool),
                "candidate_pool": candidate_pool,
                "base_candidate_id": winner.candidate_id,
                "feature_sizes": selected_by_size,
                "delta_additions": {"0": [], "1": []},
            },
        )
        for size, record in selected_by_size.items():
            self.recorder.write_json(
                f"selected_features_{size}.json",
                {
                    "status": "complete",
                    "feature_size": int(size),
                    "global_features": record["features"],
                    "selected_feature_count": int(record["feature_count"]),
