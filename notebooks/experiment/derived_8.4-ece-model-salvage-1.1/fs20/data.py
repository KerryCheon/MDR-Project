...
def drop_smap(features: list[str]) -> list[str]:
    """Apply the experiment-wide case-insensitive no-SMAP feature policy."""
...
def load_experiment_data(config: dict[str, Any]) -> ExperimentData:
    ...
    if trainval["station_id"].astype(str).nunique() != 7:
        raise ValueError("The local selector must fit exactly the seven WA stations.")
    excluded = {target, "station_id", "date", "month", "year"}
    feature_columns = drop_smap(
    ...
    v0_parent = _load_v0_features(Path(data_cfg["metadata_path"]))
    ...
    if any("smap" in feature.lower() for feature in [*feature_columns, *v0_features]):
        raise ValueError("SMAP features entered the local selector input contract.")
