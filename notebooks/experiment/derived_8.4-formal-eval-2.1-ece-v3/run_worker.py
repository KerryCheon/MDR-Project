#!/usr/bin/env python3
"""Train or evaluate a single derived_8.4-formal-eval-2.1-ece-v3 job: (config_id, seed, target).

target:
  - 'full': Temporal evaluation on Washington test set (2023-2025, 7 stations).
  - 'ece': Spatial evaluation on derived_8.4_ece_v3 (2026, 5 in-situ stations in WA).
...
def main() -> None:
    ...
    pinned_path = args.out / "pinned_configurations.json"
    if pinned_path.exists():
        pinned = json.loads(pinned_path.read_text(encoding="utf-8"))
    else:
        from eval_formal.configs import load_pinned_configs as _load_pinned

        print("[worker] pinned_configurations.json missing — rebuilding from config.yaml",
              flush=True)
        _data_tmp = load_experiment_data(PROJECT_ROOT, config)
        pinned = _load_pinned(_data_tmp, config)
    ...
    if cfg is None:
        raise SystemExit(f"config_id {args.config_id} not found in pinned_configurations.json "
                         f"({len(pinned)} pinned). Re-run run_temporal.py/run_spatial.py "
                         "to refresh the pin file.")
