#!/usr/bin/env python3
"""Temporal evaluation driver for derived_8.4-formal-eval-2.1-ece-v3 (frozen split, 30 seeds).

Validates/resumes the 20 pinned configurations x 30 seeds on the Washington test set:
...
Resume: completed jobs are skipped (meta.json + prediction/weight file presence).
"""
...
def main() -> None:
    ...
    print("=" * 70, flush=True)
    print("Temporal evaluation (frozen split, multi-seed) — derived_8.4-formal-eval-2.1-ece-v3", flush=True)
    print("=" * 70, flush=True)
    ...
    if len(seed_df) != expected:
        ...
        print(f"[Error] Incomplete temporal grid: got {len(seed_df)}/{expected}. "
