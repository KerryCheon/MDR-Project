...
def cluster_bootstrap(y_true: np.ndarray, y_pred_a: np.ndarray, y_pred_b: np.ndarray | None,
    ...
    NOTE on ECE R2: on near-constant targets (sigma_y ~ 0.003-0.008) resampled
    (station, month) blocks can have ss_tot == 0, yielding NaN R2 draws. Rank
    by RMSE; treat bootstrapped R2 CIs on ECE as descriptive only.
    """
...
def bh_fdr(pvalues: np.ndarray) -> np.ndarray:
    ...
    NaN inputs propagate as NaN (masked before ranking) so identical NaN
    placements cannot produce order-dependent q-values via ``min`` asymmetry.
    """
    ...
    for i in range(m - 1, -1, -1):
        q_i = float(ranked[i]) * m / (i + 1)
        q_i = min(q_i, q_prev)
        q_prev = q_i
        q_sorted[i] = q_i
    out_valid = np.empty(m, dtype=float)
    out_valid[order] = q_sorted
    q[valid] = out_valid
    return q
...
def run_self_tests(verbose: bool = True) -> None:
    ...
    # 4b. bh_fdr on UNSORTED input: q-values must follow original positions.
    p_unsorted = np.array([0.5, 0.001, 0.8, 0.02, 0.03])
    q_unsorted = bh_fdr(p_unsorted)
    _check("bh_fdr unsorted min p",
            abs(q_unsorted[1] - 0.001 * 5) < 1e-12)
    _check("bh_fdr unsorted matches sorted",
            bool(np.allclose(q_unsorted[np.argsort(p_unsorted)],
                             q[np.argsort(pvals)], equal_nan=True)))
    # 4c. bh_fdr with NaN: NaN propagates, valid entries unaffected by position.
    p_nan = np.array([0.02, float("nan"), 0.001])
    q_nan = bh_fdr(p_nan)
    _check("bh_fdr nan propagates", bool(np.isnan(q_nan[1])))
    _check("bh_fdr nan valid", abs(q_nan[2] - 0.001 * 2) < 1e-12)
