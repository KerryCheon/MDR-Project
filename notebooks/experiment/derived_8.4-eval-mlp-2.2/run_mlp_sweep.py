#!/usr/bin/env python3
"""Driver for the derived_8.4-eval-mlp-2.2 sweep (3 families, 3-phase seeds).

Protocol is data_version 9: train on train (2017-2020), early-stop on val
...
Three-phase, 3-seed sweep (2.2: the densest seed coverage yet — phase-2/3
top-Ns are capped at the family sizes, the direct mitigation for 2.1's
finding that the mixed/54 families' val ranking is noisy even at 3 seeds,
Spearman(val, test) = -0.309 / -0.555):
...
NEW in 2.2: `build_all_tensors` also saves `artifacts/val_meta.npz`
(y_val/year/station of the full official val set) and every job saves
best-val predictions (`val_preds.npy`, from mlp22/trainer.py) — together they
feed the offline val-year (2021 vs 2022) selection-reliability diagnostic
(`analyze_val_years.py`). No selection-rule change.

Architectures: ONLY `mlp` configs run in 2.2 — `fg`/`plr` are documented
negatives from 2.0 (best 0.782 / 0.720 vs plain-MLP 0.790) and `swa` is a
documented negative from 2.1 (0/152 deployments with the RNG guard in place);
none get GPU. The winner pool filter in the phase-2/3 selection stays
mlp/fg/plr so the protocol text is unchanged (effectively mlp-only).
...
from mlp22.data import build_feature_set, save_feature_set  # noqa: E402
