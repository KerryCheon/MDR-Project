#!/bin/bash
#SBATCH --job-name=mlp22-cpu-smoke
...
#SBATCH --nodes=1
#
# CPU-only validation of the derived_8.4-eval-mlp-2.2 pipeline under sbatch
# (no GPU; run BEFORE the GPU job to prove the compute-node env works). Runs:
#   (0a) a 3-epoch smoke sweep          -> run_mlp_sweep.py --smoke
#        (data_version -1, never reused by the real v9 run) + checks that the
#        NEW val_preds.npy / val_meta.npz artifacts are written
...
step uv run --no-sync python run_mlp_sweep.py --smoke --n-parallel 2 \
      --families 2regime_54 --only w320x320_d0.3_gelu_lr6e-4,w384x384_d0.3_gelu
