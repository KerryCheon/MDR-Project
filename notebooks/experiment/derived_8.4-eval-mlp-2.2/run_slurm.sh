#!/bin/bash
#SBATCH --job-name=mlp22-2regime-sweep
...
#SBATCH --nodes=1
#
# derived_8.4-eval-mlp-2.2 full GPU pipeline (submit from this directory):
#   mkdir -p artifacts/slurm && sbatch run_slurm.sh
#
# 1a run_mlp_sweep.py     phases 1-3, seeds 42/7/123, ~508 jobs, --resume
# 1b run_mlp_champion.py  5-seed ensembles of the val winners (per-family
#                         top-N via sweep.champion_top_n: mixed top-2 + 54
#                         top-1 + 96 top-1; extra seeds {2024, 999})
# 1c run_mlp_eval.py      leaderboard (incl. 2.1/2.0/1.3/XGB refs), per-regime,
#                         ensembles, figures
# 1d analyze_*.py         bias / selection / val-years (NEW) / overfitting /
#                         ood / stopping (tag 21)
# (the report notebook runs offline afterwards on the login node)
#
# n_parallel comes from config.yaml (8; 2.1 parity). If the node OOMs or
...
step uv run --no-sync python analyze_stopping.py --tag 21
