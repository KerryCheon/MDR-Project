#!/usr/bin/env python3
"""Aggregate the derived_8.4-eval-mlp-2.2 sweep into the eval-1.1-style report.
...
architectures (mlp / fg / plr — only mlp configs run in 2.2; fg/plr are
documented negatives from 2.0 and swa a documented negative from 2.1),
reports the selection diagnostics + Spearman
...
the mlp-1.3 rows, the mlp-2.0 rows, and NEW mlp-2.1 rows (val-selected
winners, test-best refs, val top-k ensembles, 5-seed champ, cross-family
ensemble) read from the 2.1 metrics_summary.csv via
config.mlp21_reference_file.
...
from mlp22.plots import (  # noqa: E402
    plot_diagnostics,
...
def main() -> None:
    ...
    df_summary = pd.concat([df_mlp, xgb_df, mlp13_df, mlp20_df, mlp21_df], ignore_index=True)
    ...
    df_per_reg = pd.concat([pd.DataFrame(per_reg_rows), xgb_per_reg, mlp13_per_reg, mlp20_per_reg, mlp21_per_reg], ignore_index=True)
