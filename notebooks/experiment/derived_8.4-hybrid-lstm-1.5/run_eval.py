...
def main():
    print("=" * 75, flush=True)
    print("Starting derived_8.4-hybrid-lstm-1.5 Evaluation (V21 + PCA)", flush=True)
    print("=" * 75, flush=True)
    ...
    print("\n" + "=" * 75, flush=True)
    print("FINAL MODEL LEADERBOARD (derived_8.4-hybrid-lstm-1.5)", flush=True)
    print("=" * 75, flush=True)
...
def generate_readme(df_summary: pd.DataFrame, df_regime: pd.DataFrame, shap_info: dict):
    ...
    readme_content = f"""# Experiment: `derived_8.4-hybrid-lstm-1.5` — V21 LSTM + PCA + XGBoost with Accelerated SHAP
