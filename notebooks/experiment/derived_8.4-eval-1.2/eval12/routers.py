"""Routing / Gating strategy implementations for derived_8.4-eval-1.2 (LOSO).

Same 6 routers as derived_8.4-eval-1.1. Every router exposes a `fit(train)`
method so it can be refitted per LOSO fold on the fold's trainval only
(no held-out-station leakage into routing).
"""
