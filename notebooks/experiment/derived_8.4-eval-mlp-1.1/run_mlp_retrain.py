...
on `val` (2021-2022). The XGBoost baselines train on the full trainval, so this
script retrains each family's val-selected winner on trainval for the same
...
from mlp11.data import build_feature_set  # noqa: E402
...
def main() -> None:
    ...
    for family in families:
        sub = sweep[sweep["family"] == family].sort_values("val_rmse").head(args.top_n)
        family_ens_preds: list[np.ndarray] = []
        epochs_used: list[int] = []
        for _, r in sub.iterrows():
            cid = r["config_id"]
            cfg = dict(job_cfgs[cid])
            # best epoch from the val-protocol sweep run
            ...
            for seed in seeds:
                ...
                cfg_s["data_version"] = int(config["sweep"].get("data_version", 3))
        ...
        # --- cross-config ensemble (robust to val-selection noise) ---
        if len(family_ens_preds) > 1:
            ens3 = float(np.mean(family_ens_preds, axis=0))
