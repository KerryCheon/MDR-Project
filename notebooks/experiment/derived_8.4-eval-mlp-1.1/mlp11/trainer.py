...
Protocol (data_version 3): train on `train`, early-stop on `val`, evaluate on
`test`. Extends mlp-1.0's trainer with:
  - warmup + cosine LR (LambdaLR; warmup_frac default 0.05),
  - optional EMA of the weights (decay 0.999, updated per optimizer step)
    used for val/test evaluation,
  - longer schedules (max_epochs 400, patience 60),
  - parameterized huber delta.
Keeps the mlp-1.0 checkpoint/resume contract (checkpoint.pt every
...
def train_one_config(
    ...
    *,
    ...
    train_loader, val_loader, test_loader = _build_loaders(fs, int(cfg.get("batch_size", 512)), device)
    ...
    while not done and epoch < max_epochs:
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(
                ...
                f"best={best_val_rmse:.5f} (ep{best_epoch}) test_rmse={test_rmse:.5f}",
                flush=True,
            )
    ...
    np.save(out_dir / CURVES_NAME, np.stack([np.asarray(val_curve), np.asarray(test_curve)]))
