...
def plot_sweep_summary(sweep_df: pd.DataFrame, out_dir: Path) -> Path:
    ...
    for ax, family in zip(axes, families):
        ...
        if sub.empty:
            ...
            continue
        sc = ax.scatter(sub["val_rmse"], sub["test_r2"], s=60, c="#1f77b4", alpha=0.8)
        best = sub.iloc[0]
        ax.scatter([best["val_rmse"]], [best["test_r2"]], s=160, facecolors="none",
                   edgecolors="#d62728", linewidths=2, label="best (val)")
        for _, row in sub.head(5).iterrows():
            ax.annotate(row["config_id"], (row["val_rmse"], row["test_r2"]),
                        fontsize=7, alpha=0.8, xytext=(4, 4), textcoords="offset points")
        ax.set_xlabel("Val RMSE (selection)")
        ax.set_ylabel("Test R²")
        ax.set_title(f"Sweep: {FAMILY_LABELS[family]}")
        ax.grid(True, linestyle="--", alpha=0.4)
        ax.legend(fontsize=8)
    plt.suptitle("Neural Tabular Hyperparameter Sweep (79 configs per family)", fontsize=13, weight="bold")
    plt.tight_layout()
