...
def train_v21_lstm(data_dir: Path, models_dir: Path):
    print(f"[V21] {len(ALL_FEATURES)} features, seq_len={SEQ_LEN}, {len(SEEDS)} seeds", flush=True)
    train_df = pd.read_csv(data_dir / "train.csv")
    imputer, scaler = fit_preprocessors(train_df, ALL_FEATURES)

    results = []
    for seed in SEEDS:
        val_rmse = train_one_seed(seed, ALL_FEATURES, SEQ_LEN, data_dir, models_dir, (imputer, scaler))
...
def train_v21_and_extract_all(data_dir: Path, artifacts_dir: Path, models_dir: Path):
    print("\n" + "=" * 75, flush=True)
    print("Phase 1: V21 LSTM Training (1 seed)", flush=True)
    print("=" * 75, flush=True)

    best_checkpoint, preprocessors, best_seed_info = train_v21_lstm(data_dir, models_dir)
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = load_best_model(best_checkpoint, len(ALL_FEATURES), device)
