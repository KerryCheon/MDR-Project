...
def load_hybrid_experiment_data(
    ...
    suffix = pca_suffix
    train_arr = np.load(artifacts_dir / f"{base_name}{suffix}_train.npy")
    val_arr   = np.load(artifacts_dir / f"{base_name}{suffix}_val.npy")
    test_arr  = np.load(artifacts_dir / f"{base_name}{suffix}_test.npy")

    prefix = rcfg["prefix"]
    if pca_suffix:
        prefix = f"{rcfg['prefix']}{pca_suffix.lstrip('_')}_"
