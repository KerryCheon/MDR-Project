...
def run_full_shap_analysis(
    ...
    Supports ctx_ (160-dim) and hh_ (80-dim head_hidden) feature prefixes.
    """
    ...
    for name, res in shap_results.items():
        s = res["mean_abs_shap"]
        lstm_mask = s.index.str.startswith("ctx_") | s.index.str.startswith("hh_")
    ...
    plt.suptitle("SHAP Feature Importance — derived_8.4-hybrid-lstm-1.2 (6 Models)", fontsize=14, fontweight="bold", y=1.01)
    plt.tight_layout()
