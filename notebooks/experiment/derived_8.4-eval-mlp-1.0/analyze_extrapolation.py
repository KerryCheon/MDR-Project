#!/usr/bin/env python3
"""Extrapolation (OOD) diagnostic for derived_8.4-eval-mlp-1.0.

Motivation: the hypothesis that XGBoost cannot extrapolate to feature values
unseen during training, while a (standardized) MLP can. This script flags test
rows whose top-gain backbone features fall outside the trainval [min, max]
range and compares RMSE / R² of the best MLP vs the XGBoost reference on the
in-distribution vs OOD slices.

Outputs: ood_summary.csv + ood_extrapolation.png in the experiment dir.
...
from mlp10.plots import matplotlib, plt  # noqa: E402
...
def ood_mask(frame: pd.DataFrame, feats: list[str], train_frame: pd.DataFrame) -> np.ndarray:
    ...
    for f in feats:
        ...
        outside = outside & np.isfinite(vals)  # NaN -> not OOD (missing, not unseen value)
        mask |= outside
    return mask
...
def main() -> None:
    ...
    # prediction sources: best MLP per family + XGBoost references
    ...
    for family, xgb_pred_file, xgb_label in [
        ("1regime", EVAL11_DIR / "models" / "Global_Single_backbone_0_0_preds.npy", "XGBoost Global (54)"),
        ("2regime", EVAL11_DIR / "models" / "Clustering_V0_Full_k2_winner_preds.npy", "XGBoost 2-Regime (Winner)"),
    ]:
        if family in selected.get("mlp_winners", {}):
            cid = selected["mlp_winners"][family]
            p = _load_preds(args.out / "models" / family / cid / "preds.npy")
            if p is not None:
                sources.append((f"MLP {family} ({cid})", p))
    ...
    ax.set_ylabel("RMSE")
    ax.set_title("Extrapolation check: MLP vs XGBoost on OOD test slices "
    ...
    plt.close()
    print(f"[ood] wrote ood_summary.csv + ood_extrapolation.png", flush=True)
