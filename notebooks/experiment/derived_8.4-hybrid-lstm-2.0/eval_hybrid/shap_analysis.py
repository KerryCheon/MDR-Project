...
def run_full_shap_analysis(
    ...
    # In derived_8.4-hybrid-lstm-2.0 the static-only baselines have 18 features
    # (< 20), so pad each model's list to a common length of 20.
    n_top = 20
    top20_data = {}
    for name, res in shap_results.items():
        top_s = res["mean_abs_shap"].sort_values(ascending=False).head(n_top)
        feats = list(top_s.index) + [""] * (n_top - len(top_s))
        vals = list(top_s.values) + [float("nan")] * (n_top - len(top_s))
        top20_data[f"{name}_feature"] = feats
        top20_data[f"{name}_shap_val"] = vals
    ...
    for idx, (name, res) in enumerate(shap_results.items()):
        ...
        legend_elements = [
            ...
            Patch(facecolor=colors_ctx, edgecolor="black", label="CTX (2H-dim)"),
            Patch(facecolor=colors_hh, edgecolor="black", label="Head Hidden (H-dim)"),
            Patch(facecolor=colors_hp, edgecolor="black", label="Head Pre-ReLU (H-dim)"),
        ]
    ...
    plt.suptitle(f"SHAP Feature Importance — derived_8.4-hybrid-lstm-2.0 ({n_models} XGBoost Models, Static-Only Tabular)", fontsize=14, fontweight="bold", y=1.01)
    plt.tight_layout()
