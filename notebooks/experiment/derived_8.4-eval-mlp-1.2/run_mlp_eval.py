#!/usr/bin/env python3
"""Aggregate the derived_8.4-eval-mlp-1.2 sweep into the eval-1.1-style report.
...
mean(val_rmse, aux2020_rmse)) — the selection-protocol v3 fix — and also prints
the val-only ranking + Spearman correlations so the fix is auditable. Builds
...
from mlp12.plots import (  # noqa: E402
    plot_diagnostics,
...
def load_mlp11_references(config: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Best MLP-1.1 rows per family (val-selected winner + test-best reference)."""
    ref_file = PROJECT_ROOT / Path(config["mlp11_reference_file"])
    df = pd.read_csv(ref_file)
    ...
    for fam in fam_map:
        ...
        if sub.empty:
            continue
        sub = sub.copy()
        for tag, col in [("val_sel", "val_rmse"), ("test_best", "pooled_r2")]:
            if col == "val_rmse":
                pick = sub.sort_values("val_rmse", na_position="last").head(1)
            else:
                pick = sub.sort_values("pooled_r2", ascending=False).head(1)
            r = pick.iloc[0].copy()
            r["strategy_name"] = "MLP_1.1_Reference"
            r["model_name"] = f"MLP-1.1 {FAMILY_LABELS[fam]} ({tag}: {r['config_id']})"
            keep.append(r)
    df_ref = pd.DataFrame(keep) if keep else pd.DataFrame(columns=df.columns)
    per_reg = pd.read_csv(MLP11_DIR / "per_regime_metrics_summary.csv")
    # keep only 1.1's val-selected winners per family (the reference champions)
    per_reg = per_reg[per_reg["strategy_name"].isin(["MLP_2regime_96", "MLP_2regime_54"])].copy()
    fam_winner = {"MLP_2regime_54": "res_w1024x1024_d0.2", "MLP_2regime_96": "res_w512x256x128_d0.2"}
    keep_rows = []
    for strat, winner in fam_winner.items():
        sub = per_reg[per_reg["strategy_name"] == strat]
        keep_rows.append(sub[sub["model_name"].str.contains(winner, regex=False) &
                             ~sub["model_name"].str.contains("retrain-ens", regex=False)])
    per_reg = pd.concat(keep_rows, ignore_index=True) if keep_rows else pd.DataFrame(columns=per_reg.columns)
    per_reg["strategy_name"] = "MLP_1.1_Reference"
...
def print_selection_diagnostic(sweep: pd.DataFrame, test_meta: dict, out: Path) -> None:
    ...
    print("\n" + "=" * 78, flush=True)
    print("SELECTION PROTOCOL v3 DIAGNOSTIC (robust score = mean(mean(val, aux2020)) over seeds)", flush=True)
    print("=" * 78, flush=True)
...
def main() -> None:
    ...
    mlp11_df, mlp11_per_reg = load_mlp11_references(config)
    ...
    df_summary = pd.concat([df_mlp, xgb_df, mlp11_df], ignore_index=True)
    ...
    df_per_reg = pd.concat([pd.DataFrame(per_reg_rows), xgb_per_reg, mlp11_per_reg], ignore_index=True)
    ...
    print("\n" + "=" * 72, flush=True)
    print("FINAL MODEL LEADERBOARD (derived_8.4-eval-mlp-1.2)", flush=True)
    print("=" * 72, flush=True)
