# derived_8.4-regime-interpretation-1.2
...
`Seasonal_Binary_k2`, `Univariate_G_API_k2`, `Trained_Gating_k2` — **plus a geographic &
physical deep dive of the winning `Clustering_V0_Full_k2` router** (section 3.6, new in 1.2).
It extends `derived_8.4-regime-interpretation-1.1`, whose entire per-strategy analysis is
retained verbatim; the notebook reproduces the exact eval-1.1 routers (fitted on the
`derived_8.4` train+val split) and verifies every strategy's labels against the test labels
eval-1.1 saved to disk.

The new deep-dive (section 3.6) analyzes the **geography & physical properties** of the two
`Clustering_V0_Full_k2` regimes using well-grounded domain knowledge of Washington State
hydroclimatology: per-station physiographic region, WorldClim climate normals, De Martonne and
Thornthwaite/UNEP aridity indices, HWSD soil properties, terrain, land cover, and satellite
moisture climatology. It quantifies the *commonalities within* each cluster and the *separation
between* them, and explains *why* the resulting regional-specialist experts generalize best.
A summary lives in the "Geographic & physical deep dive" section below.
...
```bash
nb execute experiment/derived_8.4-regime-interpretation-1.2/derived_8.4_regime_interpretation_1.2.ipynb --uv --timeout 900
```
...
The map cell (3.6.2b) uses `contextily` (declared in `notebooks/pyproject.toml`; `uv sync`
installs it) and downloads Esri tiles, so it needs internet at execution; the Washington
boundary is cached after the first run and tiles fall back to a vector-only map if the download
fails.
...
- **Notebook sections 3.1–3.5**: per strategy — regime sizes, month-of-year share tables,
  ...
  attributes). **Section 3.6 (new in 1.2)**: geographic & physical deep dive of
  `Clustering_V0_Full_k2` (physiographic region, WorldClim normals, De Martonne & Thornthwaite/
  UNEP aridity, HWSD soil, terrain, land cover, satellite climatology, within-cluster homogeneity
  vs between-cluster separation, and the synthesis of *why* the regimes are good). Section 4
  prints the comparison tables above (pooled, per-year and per-station spatial); section 5 is the
  synthesis.
...
## Geographic & physical deep dive (section 3.6, new in 1.2)

A focused deep dive on the winning `Clustering_V0_Full_k2` router: *what physical properties do
the two regimes share internally, and why does that make the regional-specialist experts good?*
All numbers below are the executed notebook's stdout (station-level; trainval climatologies).

### Per-station physical profile (3.6.1)

`v0full_station_physical_profile.csv` — cluster, coordinates, terrain (SRTM), WorldClim v2.1
bioclimatic normals (temps in °C, converted from °C×10), HWSD soil fractions, land cover, and
trainval satellite climatology per station. Key columns:

| station | cluster | elev (m) | BIO12 annual P (mm) | BIO14 driest P (mm) | BIO15 P seasonality | J_clay_wfrac_b100 | land cover | clim SMAP | clim NDVI |
| :--- | ---: | ---: | ---: | ---: | ---: | ---: | :--- | ---: | ---: |
| Quinault | 0 | 88 | 3349 | 60 | 62 | 17 | Tree cover | 0.598 | 0.700 |
| Paradise_WA | 0 | 1564 | 2728 | 45 | 61 | 9 | Tree cover | 0.339 | 0.262 |
| CayusePass_WA | 0 | 1588 | 2435 | 41 | 61 | 9 | Tree cover | 0.371 | 0.211 |
| Darrington | 0 | 166 | 2015 | 44 | 60 | 18 | Tree cover | 0.522 | 0.683 |
| BeaverPass_WA_990 | 0 | 1125 | 1269 | 39 | 55 | 5 | Tree cover | 0.354 | 0.537 |
| SourdoughGulch_WA_985 | 1 | 1164 | 569 | 19 | 34 | 23 | Grassland | 0.186 | 0.256 |
| Spokane | 1 | 705 | 432 | 16 | 37 | 23 | Tree cover | 0.172 | 0.387 |

### Physiographic context (3.6.2) — the Cascade rain-shadow partition

Washington's dominant hydroclimatic control is the **Cascade Range**: west of the crest,
orographic precipitation gives a cool maritime, winter-max, energy-limited climate (temperate
rainforest, conifer forest, snow-dominated basins); east of the crest, the rain shadow gives a
semi-arid continental steppe (Columbia Plateau / Palouse, loess soils, grassland / open
Ponderosa parkland) whose soil moisture is water-limited and pulse-driven. The router divides
the seven stations exactly along this divide:

- **Cluster 1 (east of crest, lon ≈ −117.5):** Spokane (Columbia Plateau / Channeled Scablands,
  705 m), SourdoughGulch_WA_985 (Palouse / Blue-Mountains foothills, 1164 m).
- **Cluster 0 (crest & west, lon ≥ −121.75):** Quinault (Olympic Peninsula rainforest, 88 m),
  Darrington (N. Cascades foothills, 166 m), BeaverPass_WA_990 (N. Cascades crest, 1125 m),
  CayusePass_WA (S. Cascades crest, 1588 m), Paradise_WA (Mount Rainier subalpine, 1564 m).

### Station location map (3.6.2b)

`v0full_station_map.png` maps the 7 stations colored by their `Clustering_V0_Full_k2` regime on
an **Esri World_Shaded_Relief** basemap (`contextily`, added to `notebooks/pyproject.toml`), so
the Cascade Range and the rain-shadow divide are directly visible. Overlays: the Washington
state outline from **Natural Earth 1:50m admin-1 states** (public domain; fetched at execution
and cached as `wa_admin1_boundary.geojson` in this experiment dir, so reruns do not re-download),
an *approximate* Cascade-crest polyline (documented reference passes/peaks: Columbia Gorge →
White/Chinook/Stampede/Snoqualmie/Stevens Passes → Rainy/Washington Passes → Mount Baker), the
wet-west / rain-shadow-east annotations, and the Esri tile attribution. The cluster markers
reuse the section-3.1 palette (c0 `#D95F02`, c1 `#1B9E77`). **Requires internet on the first
execution** (Esri tiles are fetched on every run); if the tile download fails, the cell falls
back to a vector-only map (state boundary + stations + crest) and prints a warning.

### Aridity & moisture regime (3.6.3)

De Martonne `DMI = P/(T+10)` and UNEP `AI = P/PET` (Thornthwaite PET from a cosine-reconstructed
monthly temperature cycle), from WorldClim normals:

| station | cluster | T mean (°C) | P (mm) | PET (mm) | AI = P/PET | AI class | DMI | DMI class |
| :--- | ---: | ---: | ---: | ---: | ---: | :--- | ---: | :--- |
| Quinault | 0 | 9.4 | 3349 | 542 | 6.177 | humid | 172.6 | very humid |
| Paradise_WA | 0 | 3.5 | 2728 | 402 | 6.786 | humid | 202.1 | very humid |
| CayusePass_WA | 0 | 3.4 | 2435 | 403 | 6.048 | humid | 181.7 | very humid |
| Darrington | 0 | 9.8 | 2015 | 568 | 3.551 | humid | 101.8 | very humid |
| BeaverPass_WA_990 | 0 | 4.3 | 1269 | 441 | 2.877 | humid | 88.7 | very humid |
| SourdoughGulch_WA_985 | 1 | 7.4 | 569 | 546 | 1.043 | humid | 32.7 | humid |
| Spokane | 1 | 8.4 | 432 | 573 | 0.754 | sub-humid | 23.5 | sub-humid |

Cold-climate caveat: because both indices scale moisture against temperature, the cool eastern
stations land at the sub-humid/humid boundary even though their low precipitation, summer
drought and steppe vegetation make them semi-arid by Köppen/vegetation criteria. The relative
gradient is unambiguous: **cluster-0 median AI 6.05 / DMI 172.6 / P 2435 mm vs cluster-1 median
AI 0.90 / DMI 28.1 / P 500 mm** — cluster 0 is ~4–8× wetter.

### Soil, terrain & land surface (3.6.4)

Cluster medians at the station level (`v0full_cluster_soil_terrain_landcover.csv`):

| cluster | n | clay b0 | clay b30 | clay b100 | sand b0 | sand b100 | elev (m) | slope | K_aspect_cos | SMAP | NDVI | NDMI | MSI | LST (K) |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 5 | 8 | 8 | 9 | 51 | 58 | 1205 | 18.2 | −0.50 | 0.37 | 0.537 | 0.491 | 0.368 | 281.1 |
| 1 | 2 | 21.5 | 23.5 | 23 | 33 | 35 | 929 | 11.5 | −0.88 | 0.179 | 0.322 | 0.058 | 0.933 | 290.9 |

The east sits on **loess** with clay-enriched subsoil (23 % vs 5–9 %), is drier at the surface
(lower NDVI/NDMI, much higher MSI moisture-stress index) and warmer (higher LST). Cluster 0 is
dense tree cover everywhere; cluster 1 mixes open tree cover (Spokane) and grassland
(SourdoughGulch) — *forest vs steppe*, the classic west–east vegetation gradient.

### Within-cluster homogeneity vs between-cluster separation (3.6.5)

Station-level medians, within-cluster MAD, and the between-cluster gap in units of pooled
station s.d. (`v0full_cluster_physical_commonality.csv`, top rows):

| attribute | median c0 | median c1 | within MAD | gap (pooled s.d.) |
| :--- | ---: | ---: | ---: | ---: |
| precip_seasonality_BIO15 | 61.0 | 35.5 | 1.55 | 2.28 |
| clim_F_MSI | 0.368 | 0.933 | 0.065 | 2.24 |
| clim_F_NDMI | 0.491 | 0.058 | 0.060 | 2.23 |
| aridity_index_P_over_PET | 6.048 | 0.898 | 0.726 | 2.23 |
| de_martonne_P_over_Tplus10 | 172.6 | 28.1 | 21.63 | 2.16 |
| J_clay_wfrac_b100 | 9 | 23 | 2.1 | 2.09 |
| J_sand_wfrac_b100 | 58 | 35 | 3.6 | 1.95 |
| annual_precip_P_mm | 2435 | 500.5 | 313.6 | 1.88 |
| longitude | −121.53 | −117.47 | 0.32 | 1.87 |
| clim_SMAP_sm_pm_interp | 0.37 | 0.179 | 0.046 | 1.31 |

The moisture-regime controls (climate, soil, vegetation, satellite climatology) separate by
≈ 2 pooled s.d. while the within-cluster scatter stays small — each expert sees a physically
coherent population.

### Why the regimes are good (3.6.6, synthesis)

1. **The regimes are the two physical sides of the Cascade rain shadow** — energy-limited
   forested west/crest vs water-limited semi-arid steppe east; every top separating feature is a
   *static or climatological* property (K_aspect_cos, BIO15, clay@100cm, BIO02, SMAP rollmean),
   not the target (target medians 0.239 vs 0.244) and not the calendar season.
2. **Each expert fits a simpler, more transferable feature→moisture function** because its
   population is physically homogeneous (narrow within-cluster spread, ~2 s.d. between-cluster
   gaps).
3. **The regimes differ in the actual feature→target physics** (correlation drift, e.g.
   `SMAP_sm_pm_interp_rollmean30` +1.009 across regimes): saturation/dry-down and snowmelt
   dominate the west, precipitation-pulse/temperature-driven response dominates the east.
4. **Best spatial generalization for new in-situ stations** — the geographic partition extends
   to new stations from static features alone, and the winner has the smallest LOSO gap
   (loso_mean_r2 = 0.6415, −0.1734 vs full; section 4 / eval-1.4).

## Generated artifacts

| File | Description |
| :--- | :--- |
| `derived_8.4_regime_interpretation_1.2.ipynb` | Notebook (the experiment + the 3.6 geographic/physical deep dive; explanation is this README) |
...
| `v0full_station_physical_profile.csv` | Deep-dive (3.6.1): per-station physical profile (climate, soil, terrain, land cover, satellite climatology) for `Clustering_V0_Full_k2` |
| `v0full_aridity_summary.csv` | Deep-dive (3.6.3): De Martonne & Thornthwaite/UNEP aridity indices per station |
| `v0full_cluster_soil_terrain_landcover.csv` | Deep-dive (3.6.4): soil/terrain/land-cover/land-surface medians per cluster |
| `v0full_cluster_physical_commonality.csv` | Deep-dive (3.6.5): within-cluster homogeneity vs between-cluster separation |
| `v0full_land_surface_profile.png` | Deep-dive (3.6.4): station-level satellite climatology bars by cluster |
| `v0full_physical_separation.png` | Deep-dive (3.6.4): 2×2 scatter of physical axes colored by cluster |
| `v0full_station_map.png` | Deep-dive (3.6.2b): station-location map on an Esri World_Shaded_Relief basemap (cluster-colored, Cascade crest + rain-shadow annotations) |
| `wa_admin1_boundary.geojson` | Deep-dive (3.6.2b): cached Natural Earth 1:50m admin-1 states GeoJSON (Washington polygon; public domain, fetched on first execution) |

`<strategy>` ∈ {`Clustering_V0_Full_k2`, `Clustering_Dynamic_k2`, `Seasonal_Binary_k2`,
`Univariate_G_API_k2`, `Trained_Gating_k2`}. Files marked "Deep-dive" are new in 1.2 and
concern the winning `Clustering_V0_Full_k2` router only.

## References

- `notebooks/experiment/derived_8.4-regime-interpretation-1.1/` — predecessor whose entire
  per-strategy analysis this notebook retains verbatim
- `notebooks/experiment/derived_8.4-regime-interpretation-1.0/` — original winning-router
