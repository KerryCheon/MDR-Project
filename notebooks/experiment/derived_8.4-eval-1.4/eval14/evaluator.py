"""LOSO fold-aware model evaluation for derived_8.4-eval-1.4.
...
Compared with derived_8.4-eval-1.3, the NEW gating-analysis-1.0 K-sweep
strategies route on K in {2,3,4} clusters (Backbone54 k3/k4, Static k2/k3/k4,
Weather k2/k3/k4, Dynamic k3/k4, V0_Full k3/k4) with the same
mean-impute -> StandardScaler -> KMeans recipe; their experts use ONLY the
54 shared-backbone features (no per-regime additions).
"""
...
class LosoEvaluator:
    ...
    def __init__(
        ...
        *,
        ...
        for k in range(self.n_clusters):
            self.cluster_additions.setdefault(str(k), [])
    ...
    def _router(self) -> Any:
        """Build the strategy's router (refitted per fold by the caller)."""
        return get_router(
            self.strategy_name,
            ...
            feature_sets=dict(
                (self.config.get("gating_clustering_strategies") or {})
                .get("router_feature_sets", {})
            ),
        )

    def _clusters_to_eval(self) -> tuple[int, ...]:
    ...
    def evaluate_station(self, station: str) -> LosoFoldResult:
        ...
        for cluster in self._clusters_to_eval():
