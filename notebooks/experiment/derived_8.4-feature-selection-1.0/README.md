# derived_8.4-feature-selection-1.0

Feature selection experiment on `derived_8.4` splits based on `derived_8.3-feature-selection-1.0` tooling.

## Overview

This experiment runs feature selection variants C0–C5 on `derived_8.4` and evaluates performance on the test set using the locked XGBoost protocol in `eval.ipynb`.

## Layout

| Path | Purpose |
|------|---------|
| `pipeline.ipynb` | Run selection variants C0–C5 on derived_8.4 |
...
uv run python notebooks/experiment/derived_8.4-feature-selection-1.0/run_selection.py --dataset derived_8.4

# Evaluation & Metrics:
nb execute notebooks/experiment/derived_8.4-feature-selection-1.0/eval.ipynb
```
