...
def main():
    ...
    # Read the existing content to preserve OVERALL_SELECTED_FEATURES_V1
    if dest_path.exists():
    ...
    v2_str = f"OVERALL_SELECTED_FEATURES_V2 = {repr(list(selected_feats))}"
    
    # Process file lines to update or append OVERALL_SELECTED_FEATURES_V2
    lines = content.splitlines()
    new_lines = []
    has_v2 = False
    for line in lines:
        if line.startswith("OVERALL_SELECTED_FEATURES_V2"):
            new_lines.append(v2_str)
            has_v2 = True
        else:
    ...
    if not has_v2:
        new_lines.append("")
        new_lines.append("# Selected features for overall global model - V2 (updated pipeline)")
        new_lines.append(v2_str)
    ...
    print(f"Successfully updated {dest_path} with OVERALL_SELECTED_FEATURES_V2!")
