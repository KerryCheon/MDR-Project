...
def analyze():
    v0 = set(OVERALL_SELECTED_FEATURES_V0)
    ...
    print("\n=== Category Breakdowns ===")
    all_categories = sorted(list(set(categorize(f) for f in v0 | v1 | v2 | v3 | v4 | v5)))
    
    # Let's print table of category counts for each model
    print(f"{'Category':<35} | {'V0':<4} | {'V1':<4} | {'V2':<4} | {'V3':<4} | {'V4':<4} | {'V5':<4} | {'Union':<5}")
    print("-" * 80)
    for cat in all_categories:
        count_v0 = sum(1 for f in v0 if categorize(f) == cat)
        ...
        count_union = sum(1 for f in (v0 | v1 | v2 | v3 | v4 | v5) if categorize(f) == cat)
        print(f"{cat:<35} | {count_v0:<4} | {count_v1:<4} | {count_v2:<4} | {count_v3:<4} | {count_v4:<4} | {count_v5:<4} | {count_union:<5}")
