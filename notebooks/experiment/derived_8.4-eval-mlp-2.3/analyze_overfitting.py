#!/usr/bin/env python3
"""Overfitting-symptom analysis for derived_8.4-eval-mlp-2.3 (no retraining).
...
The report notebook (derived_8.4-eval-mlp-2.3.ipynb) imports compute_overfitting
...
def compute_overfitting(sweep: pd.DataFrame, exp_dir: Path) -> dict:
    ...
    for fam in FAMILIES:
        sub = sweep[(sweep["family"] == fam) & sweep["architecture"].isin(HONEST_ARCHS)].dropna(subset=["test_r2"])
...
def print_report(r: dict) -> None:
    ...
    print("=" * 78)
    print("OVERFITTING-SYMPTOM ANALYSIS — derived_8.4-eval-mlp-2.3 (from sweep artifacts)")
    print("=" * 78)
    ...
    bias_parts = ", ".join(f"{fam}: {r[f'{fam}_med_test_bias']:.4f}" for fam in FAMILIES)
    print(f"MLP median test bias — {bias_parts}")
