#!/usr/bin/env python3
"""Generate dark-themed Station x Year metrics table cards (self-contained copy for derived_8.4-eval-mlp-1.0; original in derived_8.4-eval-1.1)."""
...
def generate_table_from_preds(
    ...
    for s in stations:
        ...
        for y in years:
            ...
            n = len(sub)
            if n > 1:
                r2 = float(r2_score(sub["soil_moisture_5cm"], sub["pred"]))
                rmse = float(root_mean_squared_error(sub["soil_moisture_5cm"], sub["pred"]))
            else:
        ...
        n_s = len(sub_s)
        if n_s > 1:
            r2_s = float(r2_score(sub_s["soil_moisture_5cm"], sub_s["pred"]))
            rmse_s = float(root_mean_squared_error(sub_s["soil_moisture_5cm"], sub_s["pred"]))
        else:
    ...
    for y in years:
        ...
        n_y = len(sub_y)
        if n_y > 1:
            r2_y = float(r2_score(sub_y["soil_moisture_5cm"], sub_y["pred"]))
            rmse_y = float(root_mean_squared_error(sub_y["soil_moisture_5cm"], sub_y["pred"]))
        else:
            r2_y, rmse_y = float("nan"), float("nan")
        cell_data["Overall"][y] = {"r2": r2_y, "rmse": rmse_y, "n": n_y}

    n_tot = len(df)
    r2_tot = float(r2_score(df["soil_moisture_5cm"], df["pred"]))
    rmse_tot = float(root_mean_squared_error(df["soil_moisture_5cm"], df["pred"]))
