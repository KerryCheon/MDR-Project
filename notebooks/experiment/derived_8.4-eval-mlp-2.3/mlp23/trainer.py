...
Protocol (data_version 10): train on `train`, early-stop on `val`, evaluate on
`test`. Byte-identical to the mlp22 trainer (data_version 9) — mlp-2.3 makes
NO training-path change, so the anchors' val curves stay bit-identical
across versions (stack check in compare_anchor_vs_2.2.py). The SWA machinery
stays importable for parity but NO 2.3 config uses it (SWA is a documented
negative — 0/152 deployments with the RNG guard in place, mlp-2.1). Train on
the OFFICIAL train split (2017-2020), early-stop / select on the OFFICIAL
val split (2021-2022), evaluate on the untouched test split (2023-2025) —
with the AUXILIARY 2020 holdout (aux) kept as a DIAGNOSTIC ONLY (mlp-1.2
documented it measures train fit, not generalization).
