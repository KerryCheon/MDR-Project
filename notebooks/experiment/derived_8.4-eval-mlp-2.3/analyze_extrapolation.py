#!/usr/bin/env python3
"""Extrapolation (OOD) diagnostic for derived_8.4-eval-mlp-2.3.
...
the best neural models per family (and the 5-seed champion ensembles) vs the
...
def main() -> None:
    ...
    for family in [f["id"] for f in config["families"]]:
        ...
        ep = args.out / "models" / "champion" / f"{family}__{cid}" / "ens_preds.npy" if cid else None
        if ep is not None and ep.exists():
            sources.append((f"MLP {FAMILY_LABELS[family]} (5-seed champ)", np.load(ep)))
