#!/usr/bin/env python3
"""Champion step for derived_8.4-eval-mlp-2.3: 5-seed ensembles of the winners.
...
seeds {2024, 999} (seeds 42, 7, 123 already exist from the sweep — the full
3-seed pool) and the 5-seed seed-mean test predictions are aggregated into:

    models/champion/<family>__<config_id>/{ens_preds.npy, ens_meta.json}

NEW in 2.3: `sweep.champion_top_n` = {2regime_mixed: 2, 2regime_54: 3,
2regime_96: 1} — the 54 family gets the deepest hedge because its val noise
is the documented problem (2.2: the 2->3-seed flip moved the 54 winner to a
worse-test config). `--top-n` may be an int (uniform, 2.1 parity) or the
per-family dict (2.2 parity).
