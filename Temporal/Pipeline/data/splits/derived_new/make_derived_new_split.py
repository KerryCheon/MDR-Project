...
SPLIT_DIR = "/Users/jbalkovec/Desktop/MDR/Temporal/Pipeline/data/splits/derived_new/"
os.makedirs(SPLIT_DIR, exist_ok=True)

TRAIN_PATH = os.path.join(SPLIT_DIR, "train_derived_new.csv")
VAL_PATH   = os.path.join(SPLIT_DIR, "val_derived_new.csv")
TEST_PATH  = os.path.join(SPLIT_DIR, "test_derived_new.csv")
META_PATH  = os.path.join(SPLIT_DIR, "split_meta_derived_new.json")
...
def _make_splits_global_temporal(df: pd.DataFrame):
    ...
    tr_min, tr_max = train_df[DATE_COL].min(), train_df[DATE_COL].max()
    va_min, va_max = val_df[DATE_COL].min(), val_df[DATE_COL].max()
    te_min, te_max = test_df[DATE_COL].min(), test_df[DATE_COL].max()

    print("\nSplit policy: GLOBAL temporal-only (all stations share same cutoffs)")
    ...
    # Optional sanity warnings: stations with missing val/test coverage
    bad = 0
    for sid in stations:
        tr_n = int((train_df[GROUP_COL] == sid).sum())
        va_n = int((val_df[GROUP_COL] == sid).sum())
        te_n = int((test_df[GROUP_COL] == sid).sum())
        if tr_n == 0 or va_n == 0 or te_n == 0:
            bad += 1

    if bad > 0:
        print(f"[WARN] {bad} station(s) have missing coverage in at least one split under GLOBAL temporal splitting.")
...
def main():
    ...
    meta = {
        ...
        "split_by": "time (global)",
        "variant": "derived_all_new_no_ground",
        "target": TARGET_COL,
