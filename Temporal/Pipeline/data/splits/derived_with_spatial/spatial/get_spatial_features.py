...
# This script fetches the spatial features for the stations in the master dataset
# These are static so they don't need any imputing (hence the separation of scripts)

# Made "generic" for future use. To run over our current stations, use:
#
# ```
# python get_spatial_features.py \
#   --coords_csv station_coords.csv \
#   --out_csv ./station_static_features.csv \
#   --project mdr-project-475522 \
#   --scale_m 250 \
#   --include_k
# ```
...
# To keep it consistent with what we've been doing so-far (naming convention)
...
def main():
    ...
    ap.add_argument("--coords_csv", required=True, help="CSV with station_id, latitude, longitude")
    ap.add_argument("--out_csv", required=True, help="Output CSV path")
    ap.add_argument("--project", default="mdr-project-475522", help="GEE project id (optional)")
    ap.add_argument("--scale_m", type=int, default=250, help="Sampling scale (meters). 250 is a good compromise.")
    ap.add_argument("--include_k", action="store_true", help="If set, add Family K derived geospatial features.")
    args = ap.parse_args()
    ...
    missing = sorted(list(required - set(stations.columns)))
    if missing:
        raise ValueError(f"coords_csv missing columns: {missing}")
