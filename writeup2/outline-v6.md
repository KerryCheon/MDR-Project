# Second Paper Outline (v6) — Covariate-Defined Regional Experts for Soil-Moisture Estimation

Status: active draft outline for PI · Split: `derived_8.4` only · Scope: Washington (regional) · CS/IEEE venue TBD
Historical predecessors: `outline-v2.md`–`outline-v5.md` (all kept unchanged) · Status of this file: active draft · v6 keeps the v5 framing and makes a final coherence pass: adds a compact end-to-end workflow, makes test-guided selection a required limitation, corrects bootstrap attribution, and aligns ECE claim labels.
...
### §7 Deployment diagnostic — ECE sensors (v5 framing; v6 Guarded-on-ECE result pending)

[Default INCLUDE as extended Discussion subsection (~0.75–1 page) + supplementary results panel T7/T9; cut both only for a strict 6-page venue — see W4. Explicitly diagnostic, NOT a SOTA-beating claim. The Guarded-on-ECE run (W4′) is pending; do not imply or report Guarded ECE outcomes until its executed report is available.]
...
- Sources: `derived_8.4-formal-eval-2.1-ece-v3` README (Tables 1–4 + time-series figs, for the tie rows); `ece-router-salvage-1.1` README (v3 pooled + station tables, for pre-Guarded oracle/poison rows); `ece-router-salvage-2.0` README (WA calibration + `auto_hard/auto_soft` deployable rows); `ece-input-diagnose-1.0` README §4–5 + figures; Guarded-on-ECE run (W4′, pending; use stdout only after execution).
...
- ECE caveats if §7 kept: 30-day late-summer dry-down window (one-sided; all dry-routed transplants collapse to 100% dry), tiny target variance (rank RMSE/bias/ubRMSE, never R²), daily aggregation hiding diurnal/event response, n = 5 stations (low power, report ns plainly), and mismatch between the trained SMAP-informed static router and unseen native-NaN SMAP in-situ inputs (hence the availability gate). `c0_only` numbers are oracle ceilings, not deployable results. The Guarded-on-ECE run remains pending W4′; after execution, label pre-Guarded vs Guarded rows and verify the dry-expert index after canonicalization.
...
| T7 | ECE routing-isolation table: routed, manual oracle, diagnostic poison, and deployable availability-gated policies by family and station | Supplementary-required; Guarded rows pending W4′ | `ece-router-salvage-1.1/2.0`; `formal-eval-2.1-ece-v3`; Guarded run after execution |
...
| T9 | ECE diagnostic panel: policy RMSE, per-station errors, routing shares, and site descriptors | New supplement; Guarded overlays pending W4′ | Salvage summaries/audits; `station_static_features.csv` |
...
| F5 | ECE time series, including Guarded overlay (`as_routed/auto_hard/c0_only`) | Supplement; Guarded overlay pending W4′ | `formal-eval-2.1-ece-v3`; salvage predictions; Guarded run after execution |
...
| C9b | ECE routing isolation: dry-expert oracle vs wet-expert poison (manual, non-deployable) | `c0_only` vs `c1_only` vs `as_routed`, frozen experts | v3 pooled RMSE `as_routed 0.165/0.166 → dry 0.059 → wet 0.191`; e.g. Lost_Meadow `0.197→0.050`, Renton_Home `0.238→0.090` | `ece-router-salvage-1.1` stdout/CSVs |
...
- **W5** Create/refresh `writeup2/` skeleton: use `outline-v6.md` as the active outline; retain `outline-v2.md` through `outline-v5.md` unchanged for history; add `claims-ledger.md`, `figures/`, and later `main.tex` or `main.md`.
...
- **W8** Reproducibility and framing gate before submission: re-execute `routing-optimize-1.0` (routing + T1/L1 cells), the pending Guarded-on-ECE run, and both figure notebooks; confirm every table row = stdout; audit router feature availability, K-selection provenance, and test-set reuse disclosures; verify `outline-v2.md` through `outline-v5.md` are unchanged; run `make lint` / `make test` if src is touched (expected: no).
